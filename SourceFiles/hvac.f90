...
CHARACTER(30), ALLOCATABLE, DIMENSION(:) :: NODE_FILTER_A,DUCT_FAN_A,DUCT_AIRCOIL_A
INTEGER :: LEAK_DUCTS = 0
INTEGER, ALLOCATABLE, DIMENSION(:,:):: LEAK_PATH
CHARACTER(255), PARAMETER :: hvacid='$Id: hvac.f90 9721 2011-12-30 18:50:45Z drjfloyd $'
CHARACTER(255), PARAMETER :: hvacrev='$Revision: 9721 $'
CHARACTER(255), PARAMETER :: hvacdate='$Date: 2011-12-30 10:50:45 -0800 (Fri, 30 Dec 2011) $'
...
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW, &
            TAU_FAN,TAU_VF,CLEAN_LOSS,LOADING(1:MAX_SPECIES),EFFICIENCY(1:MAX_SPECIES),LOADING_MULTIPLIER(1:MAX_SPECIES),FIXED_Q,&
            COOLANT_MDOT,COOLANT_CP,COOLANT_TEMPERATURE
LOGICAL :: ROUND, SQUARE, DAMPER, REVERSE, AMBIENT
CHARACTER(30) :: TYPE_ID,DEVC_ID,CTRL_ID,DUCT_ID(MAX_DUCTS),NODE_ID(2),VENT_ID,TABLE_ID,FAN_ID,AIRCOIL_ID,RAMP_ID,ID,FILTER_ID,&
                 SPEC_ID(1:MAX_SPECIES)
! CHARACTER(30) :: SUBTYPE,
...
NAMELIST /HVAC/ AIRCOIL_ID,AMBIENT,AREA,CLEAN_LOSS,COOLANT_CP,COOLANT_TEMPERATURE,COOLANT_MDOT,CTRL_ID,DAMPER,DEVC_ID,DIAMETER,&
                DUCT_ID,EFFICIENCY,FAN_ID,FILTER_ID,FIXED_Q,ID,LENGTH,LOADING,LOADING_MULTIPLIER,LOSS,MAX_FLOW,MAX_PRESSURE,&
                NODE_ID,REVERSE,RAMP_ID,ROUGHNESS,SPEC_ID,TABLE_ID,TAU_FAN,TAU_VF,TYPE_ID,VENT_ID,VOLUME_FLOW,XYZ!,ROUND, SQUARE
...
!         ENDIF
      ...
      CASE('FILTER')         
         ...
         FILTER(I_FILTER)%MULTIPLIER(1:N_TRACKED_SPECIES) = LOADING_MULTIPLIER(1:N_TRACKED_SPECIES)
      ...
      CASE('AIRCOIL')
         ...
         AIRCOIL(I_AIRCOIL)%COOLANT_CP   = COOLANT_CP
         AIRCOIL(I_AIRCOIL)%COOLANT_MDOT = COOLANT_MDOT
...
!         WRITE(*,'(A,1X,I2,1X,E12.5,1X,E12.5,1X,E12.5)') '**** ITER **** ',ITER,DT
         LHS = 0._EB
         RHS = 0._EB
         IF (N_AIRCOILS > 0) CALL COIL_UPDATE
         CALL SET_DONOR(NNE)
...
!IF (N_TRACKED_SPECIES > 0) WRITE(*,*) (TRIM(DUCTNODE(NN)%ID),DUCTNODE(NN)%ZZ(:),NN=1,N_DUCTNODES)
...
DUCT_LOOP: DO ND = 1, NE%N_DUCTS
   DU => DUCT(NE%DUCT_INDEX(ND))
   IF (DU%FIXED .OR. DU%AREA < ZERO_P) CYCLE DUCT_LOOP
...
SELECT CASE (FA%FAN_TYPE)
   CASE(1) !Constant flow
      DU%VEL(NEW) = FA%VOL_FLOW/DU%AREA*EVALUATE_RAMP(T,FA%TAU,FA%SPIN_INDEX)
   ...
   CASE(2) !Quadratic
      ...
      DEL_P = DEL_P*EVALUATE_RAMP(T,FA%TAU,FA%SPIN_INDEX)      
   CASE(3) !Fan curve
      VDOT = DU%VEL(NEW)*DU%AREA
      IF (DU%REVERSE) VDOT = -VDOT
      DEL_P = EVALUATE_RAMP(VDOT,0._EB,FA%RAMP_INDEX)*EVALUATE_RAMP(T,FA%TAU,FA%SPIN_INDEX)      
END SELECT
      ...
      DO JJ = J1,J2
         DO II = I1, I2
            ...
            IF (WC%PRESSURE_ZONE_WALL /= NODE_ZONE) THEN
               IF (NODE_ZONE == -1) THEN
                  NODE_ZONE = WC%PRESSURE_ZONE_WALL
               ELSE
...
LEAKAGE_BC: IF (LEAK_DUCTS > 0) THEN
   ...
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (SF%LEAK_PATH(1) /= WC%PRESSURE_ZONE_WALL .AND. SF%LEAK_PATH(2) /= WC%PRESSURE_ZONE_WALL) CYCLE WALL_LOOP
      IF (SF%LEAK_PATH(1) == WC%PRESSURE_ZONE_WALL) THEN
         IZ1 = SF%LEAK_PATH(1)
         IZ2 = SF%LEAK_PATH(2)
      ELSE
         IZ1 = SF%LEAK_PATH(2)
   ...
   ENDDO    
   DO IZ2 = 1,N_ZONE 
      IF (FDS_LEAK_AREA(0,IZ2,NM)<=ZERO_P .AND. FDS_LEAK_AREA(IZ2,0,NM)>=ZERO_P) THEN
         ZZ_GET = 0._EB
...
DUCT_LOOP: DO ND=1,N_DUCTS
   ...
   IF (DU%FAN_INDEX>0) THEN
      ...
      IF (DU%FAN_OPERATING .AND. FAN(DU%FAN_INDEX)%FAN_TYPE==1) THEN
         DU%FIXED=.TRUE.
         CALL UPDATE_FAN(T-DU%FAN_ON_TIME,ND)
      ENDIF              
   ENDIF
ENDDO DUCT_LOOP
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
      ...
      DO ND = 1,DN%N_DUCTS
         DU=>DUCT(DN%DUCT_INDEX(ND))
         IF (DU%FIXED .OR. DU%AREA < ZERO_P) COUNTER = COUNTER + 1
      ENDDO      
...
DO NN=1,N_DUCTNODES
   ...
   NODE_P(NN,:) = DN%P
   NODE_TMP(NN,:) = DN%TMP
   IF(DN%VENT_INDEX > 0 .AND. DN%DIR(1)*DUCT(DN%DUCT_INDEX(1))%VEL(OLD) > 0._EB) THEN
...
SUBROUTINE COIL_UPDATE
REAL(EB) :: TMP_IN,TMP_OUT,MDOT_DU,E_IN_G,CP
INTEGER :: ND
TYPE(DUCT_TYPE),POINTER::DU
...
COIL_LOOP: DO ND = 1,N_DUCTS
   ...
   IF (DU%DEVC_INDEX > 0) THEN
      IF (DEVICE(DU%DEVC_INDEX)%CURRENT_STATE) THEN
         DU%COIL_Q = 0._EB      
         CYCLE COIL_LOOP
      ENDIF
   ENDIF
   IF (DU%CTRL_INDEX > 0) THEN
      IF (DEVICE(DU%CTRL_INDEX)%CURRENT_STATE) THEN
         DU%COIL_Q = 0._EB      
         CYCLE COIL_LOOP
      ENDIF
   ENDIF
   ...
   IF (AC%FIXED_Q > -1.E9_EB) THEN
      DU%COIL_Q = -AC%FIXED_Q
   ELSE
      TMP_IN = DU%TMP_D
      CP = DU%CP_D
      MDOT_DU = DU%RHO_D*ABS(DU%VEL(GUESS))*DU%AREA
      E_IN_G = MDOT_DU*TMP_IN*CP
      TMP_OUT = (CP*MDOT_DU*TMP_IN + AC%COOLANT_TEMPERATURE*AC%COOLANT_MDOT*AC%COOLANT_CP)/&
                (CP*MDOT_DU + AC%COOLANT_MDOT*AC%COOLANT_CP)          
      DU%COIL_Q = AC%COOLANT_MDOT * AC%COOLANT_CP*(AC%COOLANT_TEMPERATURE - TMP_OUT)*AC%EFFICIENCY
   ENDIF
END DO COIL_LOOP

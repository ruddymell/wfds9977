...
DRAG_LAW_SELECT: SELECT CASE (GAS_PC%DRAG_LAW)
   ...
   CASE DEFAULT
      ...
      ! Drag reduction model, except for particles associated with a SURF line or vegetation

      WAKE_VEL=1.0_EB
!     IF (GAS_PC%SURF_INDEX<1) THEN
      IF (GAS_PC%SURF_INDEX<1 .AND. .NOT. GAS_PC%TREE) THEN
...
TREE_PARTICLES: IF (GAS_PC%TREE) THEN
   SFAC   = GAS_PC%VEG_DRAG_CONSTANT*GAS_P%VEG_SV*GAS_P%VEG_PACKING_RATIO*QREL*C_DRAG*GAS_P%VEG_VOLFRACTION
   GAS_P%ACCEL_X  = SFAC*UREL !+ GAS_P%VEG_MLR*UREL/RHO_G !was removed because it appears to be
   GAS_P%ACCEL_Y  = SFAC*VREL !+ GAS_P%VEG_MLR*VREL/RHO_G !double counting of the expansion
   GAS_P%ACCEL_Z  = SFAC*WREL !+ GAS_P%VEG_MLR*WREL/RHO_G !which is already in the velocity divergence

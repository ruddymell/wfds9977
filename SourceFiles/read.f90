...
INTEGER :: N_CFCR_TREES,NDIM,N_IGNITORS
...
NAMELIST /TREE/ CROWN_BASE_HEIGHT,CROWN_WIDTH,CROWN_WIDTH_BOTTOM,CROWN_WIDTH_TOP,FUEL_GEOM,IGNITOR_ELEMENTS,LABEL,OUTPUT_TREE,&
                ...
                T_RAMPOFF_IGNITOR_ELEMENTS,T_RAMPON_IGNITOR_ELEMENTS,XB,XYZ,TREE_NAME
...
PRINT '(A,1x,6i8)','read: N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_RING_TREES', &
N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_RING_TREES
...
IF (N_TREES_OUT /= 0) THEN
 ...
 ALLOCATE(TREE_OUTPUT_DATA(N_TREES_OUT,11,NMESHES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_OUTPUT_DATA',IZERO)
 ALLOCATE(TREE_OUTPUT_DATA_TOTAL(N_TREES_OUT,11),STAT=IZERO)
...
NDIM = N_CONE_TREES + N_FRUSTUM_TREES + N_CYLINDER_TREES + N_RING_TREES
IF (NDIM > 0) THEN
...
IF (NDIM > 0) THEN
 ...
 CALL ChkMemErr('READ','TREE_RECT_INDEX',IZERO)
...
!
! Specifiy veg geometry spedific quantities
!
   IF (FUEL_GEOM=='CONE' .OR. FUEL_GEOM=='CYLINDER' .OR. FUEL_GEOM=='RING') THEN

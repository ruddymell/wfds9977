...
!     WRITE(LU_ERR,'(/A)') "Fire Dynamics Simulator"
      WRITE(LU_ERR,'(/A)')  "WFDS9977; based on subversion 9977 (2012) of FDS"
...
!     COMPILE_DATE = 'September 20, 2019'
      WRITE(LU_ERR,'(A,A)') "Source Date: ",'January 16, 2022'
...
CALL READ_BRNR    ! read names of files containing HRRPUA for "burner" emulation of fireline spread
CALL READ_VOLU
CALL READ_OBST
CALL READ_VENT
CALL READ_ZONE
...
REAL(EB) :: DT,VEL_CHAR
INTEGER :: NM
NAMELIST /TIME/ DT,EVAC_DT_FLOWFIELD,EVAC_DT_STEADY_STATE,FYI,LOCK_TIME_STEP,RESTRICT_TIME_STEP,SYNCHRONIZE,T_BEGIN,T_END,&
                TIME_SHRINK_FACTOR,WALL_INCREMENT, &
...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,CHECK_GR,CHECK_HT,CHECK_VN,CLIP_MASS_FRACTION,CP_FTMP,&
                CONSTANT_SPECIFIC_HEAT,C_SMAGORINSKY,C_DEARDORFF,C_VREMAN,C_G,C_FORCED,&
                ...
                OVERWRITE,PARTICLE_CFL,PARTICLE_CFL_MAX,PERIODIC_TEST,POROUS_FLOOR,PR,PROJECTION,P_INF,&
                RAMP_GX,RAMP_GY,RAMP_GZ,&
                RESTART,RESTART_CHID,RFAC_FORCING, RUN_AVG_FAC,SC,SHARED_FILE_SYSTEM,SMOKE_ALBEDO,SOLID_PHASE_ONLY,&
                STRATIFICATION,TENSOR_DIFFUSIVITY,TERRAIN_CASE,TERRAIN_IMAGE,TEXTURE_ORIGIN, &
                THERMOPHORETIC_DEPOSITION,&
                THICKEN_OBSTRUCTIONS,TMPA,TURBULENT_DEPOSITION,TURBULENCE_MODEL,U0,UVW_FILE,V0,VEG_LEVEL_SET_BURNERS_FOR_FIRELINE, &
                VEG_LEVEL_SET_COUPLED,VEG_LEVEL_SET_SURFACE_HRRPUA, &
                VEG_LEVEL_SET_UNCOUPLED,VEG_LEVEL_SET_SURFACE_HEATFLUX,VEG_LEVEL_SET_THERMAL_ELEMENTS,LEVEL_SET_MODE, &
                VISIBILITY_FACTOR,VN_MAX,VN_MIN,W0,WFDS,WIND_ONLY,PLE_UATM,U_Z0_UATM,Z0_UATM,VEG_INPUT_FILENAME,CREATE_VEG_FILE, &
...
VN_MIN               = 0.8_EB
VEG_LEVEL_SET           = .FALSE.
VEG_LEVEL_SET_BURNERS_FOR_FIRELINE = .FALSE.
...
VEG_LEVEL_SET_SURFACE_HRRPUA   = .FALSE.
CREATE_VEG_FILE                = .FALSE.
VEG_INPUT_FILENAME             = 'null'
...
NAMELIST /DUMP/ COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEBUG,DEVC_COLUMN_LIMIT,DT_BNDE,DT_BNDF,DT_CTRL,DT_DEVC,DT_DEVC_LINE,DT_FLUSH,&
                DT_GEOM,DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RESTART,DT_SL3D,DT_SLCF,DT_VEG,DT_OUTPUT_LS,EB_PART_FILE,&
                FLUSH_FILE_BUFFERS,&
...
IF (DT_BNDE < 0._EB) DT_BNDE = 2._EB * (T_END - T_BEGIN)/REAL(NFRAMES,EB)
...
IF (DT_OUTPUT_LS  < 0._EB) DT_OUTPUT_LS  = (T_END - T_BEGIN)/REAL(NFRAMES,EB)

IF (DT_GEOM < 0._EB) DT_GEOM = 1.E10_EB
...
SPEC_READ_LOOP: DO N=1,N_SPECIES
   ...
   SELECT CASE (ID)
      ...
      CASE('SOOT')
         SOOT_INDEX = N
         IF (SIMPLE_CHEMISTRY .AND. SOOT_DEPOSITION) AEROSOL = .TRUE.
...
IF (SIMPLE_CHEMISTRY) THEN
   ...
   SOOT_IF:IF (N<=1) THEN
      ...
      END SELECT
   ELSEIF (.NOT. SOOT_DEPOSITION .AND. N==2) THEN SOOT_IF
      ID                 = 'PRODUCTS'
      FORMULA            = 'Z2'
   ...
   ELSEIF(SOOT_DEPOSITION .AND. (N==2 .OR. N==3)) THEN SOOT_IF
      RN%NU_CO           = (SPECIES(FUEL_INDEX)%MW/MW_CO)     *RN%CO_YIELD
      RN%NU_SOOT         = (SPECIES(FUEL_INDEX)%MW/RN%MW_SOOT)*RN%SOOT_YIELD
      RN%NU_H2O          = 0.5_EB*RN%H - 0.5_EB*RN%NU_SOOT*RN%SOOT_H_FRACTION
      RN%NU_CO2          = RN%C - RN%NU_CO - RN%NU_SOOT*(1._EB-RN%SOOT_H_FRACTION)
      ...
      IF (N==2) THEN
         ...
         SPEC_ID(4)         = 'NITROGEN'
         VOLUME_FRACTION(1) = RN%NU_CO
         VOLUME_FRACTION(2) = RN%NU_H2O + SPECIES_MIXTURE(0)%VOLUME_FRACTION(H2O_INDEX)*RN%NU_O2 / &
                                          SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)
         VOLUME_FRACTION(3) = RN%NU_CO2 + SPECIES_MIXTURE(0)%VOLUME_FRACTION(CO2_INDEX)*RN%NU_O2 / &
                                          SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)
         VOLUME_FRACTION(4) = RN%NU_N2  + SPECIES_MIXTURE(0)%VOLUME_FRACTION(N2_INDEX)*RN%NU_O2 / &
                                          SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)
         VOLUME_FRACTION    = VOLUME_FRACTION/SUM(VOLUME_FRACTION)
      ELSE 
         ID                 = 'SOOT'
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,EPUMO2,A, &
            ...
            AUTO_IGNITION_TEMPERATURE,THRESHOLD_TEMPERATURE,HUMIDITY,SOOT_H_FRACTION,N_T,TOL_INT
REAL(EB) :: E_TMP=0._EB,S_TMP=0._EB,ATOM_COUNTS(118),MW_FUEL=0._EB
LOGICAL :: A_TMP,L_TMP,CHECK_ATOM_BALANCE,REVERSIBLE,NEW_MIX
NAMELIST /REAC/ A,AUTO_IGNITION_TEMPERATURE,BETA_EDC,C,CHECK_ATOM_BALANCE,CO_PRODUCTION,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                E,EPUMO2,EQUATION,FIXED_MIX_TIME,FORMULA,FUEL,FWD_ID,FYI,H,HEAT_OF_COMBUSTION,HRRPUA_SHEET,HRRPUV_AVERAGE,&
                HUMIDITY,ID,IDEAL,N,NEW_MIX,NU,N_S,N_T,O,ODE_SOLVER,REAC_ATOM_ERROR,REAC_MASS_ERROR,REVERSIBLE,SMIX_ID,&
...
IF (SIMPLE_CHEMISTRY) THEN
   RN => REACTION(1)
   IF (RN%SOOT_YIELD < ZERO_P .AND. SOOT_DEPOSITION) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Cannot specify soot deposition with SOOT_YIELD=0'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   RN%SMIX_ID_READ(1) = RN%FUEL
   RN%SMIX_ID_READ(2) = 'AIR'
   RN%SMIX_ID_READ(3) = 'PRODUCTS'
   IF (SOOT_DEPOSITION) RN%SMIX_ID_READ(4) = 'SOOT'
   RN%NU_READ(1)      = -1._EB
   RN%NU_READ(2)      = -RN%NU_O2*SPECIES_MIXTURE(1)%VOLUME_FRACTION(FUEL_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)
   IF (SOOT_DEPOSITION) THEN
      RN%N_SMIX          = 4
      RN%NU_READ(4)      = RN%SOOT_YIELD*SPECIES_MIXTURE(1)%MW/SPECIES_MIXTURE(3)%MW
      RN%NU_READ(3)      = -(RN%SOOT_YIELD*SPECIES_MIXTURE(1)%MW + &
                             RN%NU_READ(1)*SPECIES_MIXTURE(1)%MW+RN%NU_READ(2)*SPECIES_MIXTURE(0)%MW)/SPECIES_MIXTURE(2)%MW
   ELSE
      RN%N_SMIX          = 3
...
INTEGER :: SAMPLING_FACTOR,N,NN,IPC,RGB(3),I_DUMMY(6),NUMBER_RADIATION_ANGLES
INTEGER :: VEG_NDT_SUBCYCLES
...
REAL(EB) :: VEG_SV,VEG_MOISTURE,VEG_CHAR_FRACTION,VEG_DRAG_CONSTANT,VEG_DENSITY,VEG_BULK_DENSITY, &
            VEG_BURNING_RATE_MAX,VEG_CHAROX_RATE_MAX,VEG_DEHYDRATION_RATE_MAX,VEG_INITIAL_TEMPERATURE, &
            VEG_FUEL_MPV_MIN,VEG_MOIST_MPV_MIN,USER_DRAG_COEFFICIENT,VEG_CHAR_ENTHALPY_FRACTION,VEG_ASH_FRACTION
REAL(EB) :: VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
            VEG_BETA_CHAR,VEG_NU_O2_CHAR
REAL(EB) :: TE_BURNTIME
...
CHARACTER(30) :: SPEC_ID,DEVC_ID,CTRL_ID,QUANTITIES(1:10),SURF_ID,DRAG_LAW,PROP_ID,RADIATIVE_PROPERTY_TABLE='null',&
                 CNF_RAMP_ID='null',CHILD_CNF_RAMP_ID='null',DISTRIBUTION,CHILD_DISTRIBUTION
CHARACTER(25) :: COLOR
CHARACTER(25) :: VEG_DEGRADATION
LOGICAL :: MASSLESS,STATIC,TREE,MONODISPERSE,EVAPORATE,BREAKUP,CHECK_DISTRIBUTION,FUEL,WATER
LOGICAL :: VEG_REMOVE_CHARRED,VEG_STEM,VEG_CHAR_OXIDATION,VEG_IGNITED,VEG_HCONV_CYLLAM,VEG_KEEP_FOR_SMV
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC=>NULL()
TYPE(PROPERTY_TYPE), POINTER :: PY=>NULL()
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CHILD_DIAMETER,BREAKUP_CHILD_GAMMA_D,BREAKUP_CHILD_SIGMA_D,CHECK_DISTRIBUTION,&
                CHILD_CNF_RAMP_ID,CHILD_DISTRIBUTION,CNF_RAMP_ID,COLOR,COMPLEX_REFRACTIVE_INDEX,CTRL_ID,DENSE_VOLUME_FRACTION,&
                DENSITY,DEVC_ID,DIAMETER,DISTRIBUTION,DRAG_LAW,EVAPORATE,FYI,GAMMA_D,HEAT_OF_COMBUSTION,&
                HORIZONTAL_VELOCITY,ID,INITIAL_TEMPERATURE,MASSLESS,MAXIMUM_DIAMETER,MINIMUM_DIAMETER,MONODISPERSE,&
                NUMBER_RADIATION_ANGLES,ORIENTATION,PROP_ID,QUANTITIES,RADIATIVE_PROPERTY_TABLE,REAL_REFRACTIVE_INDEX,RGB,&
                SAMPLING_FACTOR,SIGMA_D,SPEC_ID,STATIC,SURFACE_TENSION,SURF_ID,TREE,USER_DRAG_COEFFICIENT,VEG_BULK_DENSITY,&
                VEG_BURNING_RATE_MAX,VEG_CHAR_FRACTION,VEG_CHAR_OXIDATION,VEG_DEGRADATION,VEG_DEHYDRATION_RATE_MAX,VEG_DENSITY,&
                VEG_DRAG_CONSTANT,VEG_FUEL_MPV_MIN,VEG_INITIAL_TEMPERATURE,VEG_MOISTURE,VEG_MOIST_MPV_MIN,VEG_REMOVE_CHARRED,&
                VEG_STEM,VEG_SV,VERTICAL_VELOCITY,&
                FUEL,WATER,VEG_CHAR_ENTHALPY_FRACTION,VEG_ASH_FRACTION, & ! Backward compatibility
                VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
                VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_NDT_SUBCYCLES,VEG_CHAROX_RATE_MAX,TE_BURNTIME,VEG_HCONV_CYLLAM, &
                VEG_KEEP_FOR_SMV
...
COUNT_PART_LOOP: DO
   ...
   N_PART = N_PART + 1
   220 IF (IOS>0) THEN
    PRINT*,'Erro: Probably PART number and ID =',N_PART+1, ID
    CALL SHUTDOWN('ERROR: Problem with PART line')
   ENDIF
ENDDO COUNT_PART_LOOP
219 REWIND(LU_INPUT)

IF (N_PART>0) PARTICLE_FILE = .TRUE.
...
READ_PART_LOOP: DO N=1,N_PART
   ...
   VEG_SV                   = 4000. ! 1/m
   VEG_MOISTURE             = 1.0_EB
   VEG_CHAR_FRACTION        = 0.20_EB
   VEG_ASH_FRACTION         = 0.03_EB !fraction of original dry fuel mass that can become ash
   VEG_DRAG_CONSTANT        = 1.0_EB
   VEG_HCONV_CYLLAM         = .FALSE.
   VEG_DENSITY              = 540._EB ! kg/m3 dry mass
   VEG_BULK_DENSITY         = 0.3_EB  ! kg/m3
   VEG_BURNING_RATE_MAX     = 0.4_EB  ! kg/m3/s
   VEG_CHAROX_RATE_MAX      = 999._EB ! kg/m3/s
   VEG_DEHYDRATION_RATE_MAX = 0.4_EB  ! kg/m3/s
   VEG_INITIAL_TEMPERATURE  = TMPA - TMPM  ! C TMPM is subtracted because it's added back in in sub allocate_wall_arrays
   VEG_FUEL_MPV_MIN         = 0.001_EB*VEG_BULK_DENSITY
   VEG_MOIST_MPV_MIN        = VEG_MOISTURE*VEG_FUEL_MPV_MIN
   ...
   CHILD_DISTRIBUTION       = 'ROSIN-RAMMLER-LOGNORMAL'
   CHILD_CNF_RAMP_ID        = 'null'
   WATER                    = .FALSE. ! Backward compatibility
   FUEL                     = .FALSE. ! Backward compatibility
   VEG_CHAR_ENTHALPY_FRACTION = 0.5_EB
   VEG_IGNITED              = .FALSE.
   VEG_H_H2O                = 2259._EB !kJ/kg heat of water vaporization
   VEG_A_H2O                = 600000._EB !/1/s sqrt(K)
   VEG_E_H2O                = 5800._EB !K
   VEG_H_PYR                = 418._EB !kJ/kg Porterie, Grishin
   VEG_A_PYR                = 36300._EB !1/s Porterie, Grishin
   VEG_E_PYR                = 7250._EB  !K Porterie, Grishin
   VEG_H_CHAR               = -12.0E+3_EB !kJ/kg Porterie, Grishin
   ...
   VEG_KEEP_FOR_SMV         = .FALSE. !IF T, then consumed veg is kept with no mass and no drag
   ...
   ! Process SAMPLING_FACTOR

   IF (SAMPLING_FACTOR<=0) THEN
      SAMPLING_FACTOR = 10
      IF (MASSLESS .OR. SURF_ID/='null')  SAMPLING_FACTOR = 1
   ENDIF
   ...
   IF (PC%SPEC_ID/='null') THEN
      ...
      DO NN=0,N_TRACKED_SPECIES
         IF (TRIM(SPECIES_MIXTURE(NN)%ID)==TRIM(PC%SPEC_ID)) THEN
            PC%Z_INDEX = NN
            EXIT
         ENDIF
      ENDDO
      ...
      ENDIF
      DO NN=1,N_SPECIES
         IF (SPECIES_MIXTURE(PC%Z_INDEX)%MASS_FRACTION(NN)>0._EB) THEN
            IF (PC%Y_INDEX > 0) THEN
               WRITE(MESSAGE,'(A,I3,A)') 'ERROR: PART line ',N,'.  Particles cannont evaporate to a lumped species.'
               CALL SHUTDOWN(MESSAGE)            
            ELSE
               PC%Y_INDEX = NN        
            ENDIF
   ...
   IF (RADIATIVE_PROPERTY_TABLE /= 'null') THEN
      CALL GET_TABLE_INDEX(RADIATIVE_PROPERTY_TABLE,PART_RADIATIVE_PROPERTY,PC%RADIATIVE_PROPERTY_INDEX)
      PC%RADIATIVE_PROPERTY_TABLE_ID = RADIATIVE_PROPERTY_TABLE
   ELSE
   ...
   PC%CHECK_DISTRIBUTION = CHECK_DISTRIBUTION
   PC%CHILD_DISTRIBUTION = CHILD_DISTRIBUTION
   PC%CNF_RAMP_ID        = CNF_RAMP_ID
   PC%CHILD_CNF_RAMP_ID  = CHILD_CNF_RAMP_ID
   
   IF(PC%CNF_RAMP_ID/='null') THEN
        CALL GET_RAMP_INDEX(PC%CNF_RAMP_ID,'DIAMETER',PC%CNF_RAMP_INDEX)
   ENDIF
   ! Get size distribution CNF ramp
   IF(PC%CHILD_CNF_RAMP_ID/='null') THEN
        CALL GET_RAMP_INDEX(PC%CHILD_CNF_RAMP_ID,'DIAMETER',PC%CHILD_CNF_RAMP_INDEX)
   ENDIF
   ...
   PC%VEG_ASH_FRACTION         = VEG_ASH_FRACTION !fraction of original dry fuel mass
   PC%VEG_DRAG_CONSTANT        = VEG_DRAG_CONSTANT   
   PC%VEG_HCONV_CYLLAM         = VEG_HCONV_CYLLAM
   ...
   PC%VEG_CHAROX_RATE_MAX      = VEG_CHAROX_RATE_MAX !kg/m3/s
   PC%VEG_DEHYDRATION_RATE_MAX = VEG_DEHYDRATION_RATE_MAX !kg/m3/s
   PC%VEG_INITIAL_TEMPERATURE  = VEG_INITIAL_TEMPERATURE +TMPM ! K
   PC%VEG_FUEL_MPV_MIN         = 0.001_EB*VEG_BULK_DENSITY
   PC%VEG_MOIST_MPV_MIN        = VEG_MOISTURE*PC%VEG_FUEL_MPV_MIN
...
!  PC%VEG_CHAR_MPV_MIN         = VEG_CHAR_FRACTION*PC%VEG_FUEL_MPV_MIN !determined in vege.f90
   ...
   PC%VEG_E_H2O                = VEG_E_H2O
   PC%VEG_H_PYR                = VEG_H_PYR*1000._EB !J/kg 
   PC%VEG_A_PYR                = VEG_A_PYR 
   PC%VEG_E_PYR                = VEG_E_PYR 
   PC%VEG_H_CHAR               = VEG_H_CHAR*1000._EB !J/kg 
   PC%VEG_A_CHAR               = VEG_A_CHAR
   PC%VEG_E_CHAR               = VEG_E_CHAR 
   PC%VEG_BETA_CHAR            = VEG_BETA_CHAR
   ...
   PC%VEG_KEEP_FOR_SMV         = VEG_KEEP_FOR_SMV
   ...
   IF (PC%N_SPLIT>1) THEN   
      ...
      DO NN = 1,PC%N_SPLIT
         ...
         PC%ORIENTATION(NN,1:3)   = ORIENTATION(NN,1:3)/ NORM2(OR_TEMP)
      ENDDO
   ENDIF
   ...
   IF (ANY(PC%QUANTITIES/='null')) THEN
      QUANTITIES_LOOP: DO NN=1,10
         ...
         CALL GET_QUANTITY_INDEX(PC%SMOKEVIEW_LABEL(PC%N_QUANTITIES),PC%SMOKEVIEW_BAR_LABEL(PC%N_QUANTITIES), &
                                 PC%QUANTITIES_INDEX(PC%N_QUANTITIES),I_DUMMY(1), &
                                 I_DUMMY(2),I_DUMMY(3),I_DUMMY(4),I_DUMMY(5),I_DUMMY(6),'PART', &
                                 PC%QUANTITIES(NN),'null','null','null','null','null')   
...
INTEGER :: N, J, ITMP
REAL(EB) :: H_L,H_V,CPBAR,H_G_S,H_G_S_REF,H_L_REF,TMP_REF,TMP_MELT,TMP_V,TMP_WGT,DENSITY
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC=>NULL()
TYPE(SPECIES_TYPE),POINTER:: SS=>NULL()
...
PART_LOOP: DO N=1,N_PART
   ...
   ! Only process particles or PARTICLEs that have mass and evaporate
...
INTEGER :: N_CFCR_TREES,NDIM,N_IGNITORS,N_NON_IN_FILE_TREES
...
LOGICAL  :: IGNITOR_ELEMENTS,OUTPUT_TREE,TREE_IN_FILE
CHARACTER(30) :: PART_ID,LABEL,TREE_NAME 
...
NAMELIST /TREE/ CROWN_BASE_HEIGHT,CROWN_WIDTH,CROWN_WIDTH_BOTTOM,CROWN_WIDTH_TOP,FUEL_GEOM,IGNITOR_ELEMENTS,LABEL,OUTPUT_TREE,&
                ...
                T_RAMPOFF_IGNITOR_ELEMENTS,T_RAMPON_IGNITOR_ELEMENTS,XB,XYZ,TREE_NAME,TREE_IN_FILE

!Bin file testing ->
!For making a binary file with veg info to test the reading in of such a binary file
!OPEN(8137,FILE=TRIM(CHID)//'_output_veg.bin',FORM='UNFORMATTED',STATUS='NEW')
!Bin file testing <-
...
N_RECTANGLE_TREES    = 0
N_RING_TREES         = 0
N_TREES_OUTPUT_DATA  = 0
N_CONE_IGNITORS      = 0
N_FRUSTUM_IGNITORS   = 0
N_CYLINDER_IGNITORS  = 0
N_RECTANGLE_IGNITORS = 0
N_RING_IGNITORS      = 0
N_IGNITORS           = 0
N_TREES_IN_FILE      = 0
N_NON_IN_FILE_TREES  = 0
...
COUNT_VEG_LOOP: DO
   ...
   N_TREES = N_TREES + 1
   IF (FUEL_GEOM == 'CONE')       N_CONE_TREES        = N_CONE_TREES + 1
   IF (FUEL_GEOM == 'FRUSTUM')    N_FRUSTUM_TREES     = N_FRUSTUM_TREES + 1
   IF (FUEL_GEOM == 'CYLINDER')   N_CYLINDER_TREES    = N_CYLINDER_TREES + 1
   IF (FUEL_GEOM == 'RECTANGLE')  N_RECTANGLE_TREES   = N_RECTANGLE_TREES + 1
   IF (FUEL_GEOM == 'RING')       N_RING_TREES        = N_RING_TREES + 1
   IF (IGNITOR_ELEMENTS)          N_IGNITORS          = N_IGNITORS + 1
   IF (OUTPUT_TREE .AND. LABEL /= 'null') N_TREES_OUTPUT_DATA = N_TREES_OUTPUT_DATA + 1
   IF (TREE_IN_FILE)              N_TREES_IN_FILE     = N_TREES_IN_FILE + 1
   N_NON_IN_FILE_TREES = N_CONE_TREES + N_FRUSTUM_TREES + N_CYLINDER_TREES + N_RECTANGLE_TREES + N_RING_TREES + N_IGNITORS
   IF (OUTPUT_TREE .AND. TREE_IN_FILE .AND. N_NON_IN_FILE_TREES > 0) THEN
     WRITE(MESSAGE,'(A)') 'ERROR: All &TREE lines with TREE_IN_FILE=T must be the first &TREE lines in the input file'
     CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (TREE_NAME == 'null' .AND. CREATE_VEG_FILE) THEN
     WRITE(MESSAGE,'(A)') 'ERROR: CREATE_VEG_FILE=T so must provide TREE_NAME in &TREE'
     CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (CREATE_VEG_FILE .AND. .NOT. OUTPUT_TREE) THEN
     WRITE(MESSAGE,'(A)') 'ERROR: When creating a veg file, set OUTPUT_TREE=T in &TREE'
     CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (VEG_INPUT_FILENAME /= 'null' .AND. .NOT. OUTPUT_TREE) THEN
     WRITE(MESSAGE,'(A)') 'ERROR: When reading in a veg file, set OUTPUT_TREE=T in &TREE'
     CALL SHUTDOWN(MESSAGE)
   ENDIF
   12 IF (IOS>0) THEN 
...
11 REWIND(LU_INPUT)
!
! Sequentially read the TREE namelist to get shape and size parameters for each vegetation volume.
!
ALLOCATE(TREE_MESH(NMESHES),STAT=IZERO)
CALL ChkMemErr('READ','TREE_MESH',IZERO); TREE_MESH = .FALSE.
...
ENDIF
!
!print*,'read: N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_RING_TREES', &
!N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_RING_TREES

PRINT '(A,1x,7i8)','read: N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_RING_TREES,N_TREES_IN_FILE', &
N_TREES,N_CONE_TREES,N_FRUSTUM_TREES,N_CYLINDER_TREES,N_RECTANGLE_TREES,N_RING_TREES,N_TREES_IN_FILE
...
IF (N_TREES_OUTPUT_DATA /= 0) THEN
 ...
 ALLOCATE(TREE_OUTPUT_DATA(N_TREES_OUTPUT_DATA,11,NMESHES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_OUTPUT_DATA',IZERO)
 ALLOCATE(TREE_OUTPUT_DATA_TOTAL(N_TREES_OUTPUT_DATA,11),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_OUTPUT_DATA_TOTAL',IZERO)
ENDIF

!-- Allocated arrays to hold veg array indices and veg geometry
!   dependent arrays that define the dimensions of the veg
ALLOCATE(VEG_FUEL_GEOM(N_TREES),STAT=IZERO)
CALL ChkMemErr('READ','VEG_FUEL_GEOM',IZERO)

!for array index for parameters shared by cone,frustum,cylinder,ring veg volumes 
NDIM = N_CONE_TREES + N_FRUSTUM_TREES + N_CYLINDER_TREES + N_RING_TREES + N_TREES_IN_FILE
IF (NDIM > 0) THEN
 ALLOCATE(TREE_CFCR_INDEX(N_TREES),STAT=IZERO) 
 CALL ChkMemErr('READ','TREE_CRCR_INDEX',IZERO)
 ALLOCATE(X_TREE(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','X_TREE',IZERO)
 ALLOCATE(Y_TREE(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','Y_TREE',IZERO)
 ALLOCATE(Z_TREE(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','Z_TREE',IZERO)
 ALLOCATE(TREE_H(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_H',IZERO)
 ALLOCATE(CROWN_B_H(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','CROWN_B_H',IZERO)
 ALLOCATE(CROWN_W(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','CROWN_W',IZERO)
ENDIF

!for array index for parameters only for frustum veg volume
!and array that hold frustum volume specific parameters
NDIM = N_FRUSTUM_TREES
IF (NDIM > 0) THEN
 ALLOCATE(TREE_FRUSTUM_INDEX(N_TREES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_FRUSTRUM_INDEX',IZERO)
 ALLOCATE(CROWN_W_TOP(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','CROWN_W_TOP',IZERO)
 ALLOCATE(CROWN_W_BOTTOM(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','CROWN_W_BOTTOM',IZERO)
ENDIF

!for array index used to find parameters only for rectangular veg volume
!arrays for rectangular volume specific parameters
NDIM = N_RECTANGLE_TREES
IF (NDIM > 0) THEN
 ALLOCATE(TREE_RECT_INDEX(N_TREES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_RECT_INDEX',IZERO) ; TREE_RECT_INDEX = 0
 ALLOCATE(XS_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','XS_RECT_VEG',IZERO)
 ALLOCATE(XF_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','XF_RECT_VEG',IZERO)
 ALLOCATE(YS_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','YS_RECT_VEG',IZERO)
 ALLOCATE(YF_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','YF_RECT_VEG',IZERO)
 ALLOCATE(ZS_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','ZS_RECT_VEG',IZERO)
 ALLOCATE(ZF_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','ZF_RECT_VEG',IZERO)
ENDIF

!for array index for parameters only for ring veg volume 
!array defining ring volume specific parameter
NDIM = N_RING_TREES
IF (NDIM > 0) THEN
 ALLOCATE(TREE_RING_INDEX(N_TREES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_RING_INDEX',IZERO)
 ALLOCATE(RING_THICKNESS_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','RING_THICKNESS_VEG',IZERO)
ENDIF

!for array index for ignitor vegetation
!arrays defining ignitor
ALLOCATE(IGN_ELEMS(N_TREES),STAT=IZERO)
CALL ChkMemErr('READ','IGN_ELEMS',IZERO)
NDIM = N_IGNITORS
IF (N_IGNITORS >0) THEN
 ALLOCATE(TREE_IGN_INDEX(N_TREES),STAT=IZERO)
 CALL ChkMemErr('READ','TREE_IGN_INDEX',IZERO)
 ALLOCATE(TON_IGN_ELEMS(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','TON_IGN_ELEM',IZERO)
 ALLOCATE(TOFF_IGN_ELEMS(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','TOFF_IGN_ELEM',IZERO)
 ALLOCATE(T_RAMPOFF_IGN_ELEMS(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','T_RAMPOFF_IGN_ELEM',IZERO)
 ALLOCATE(T_RAMPON_IGN_ELEMS(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','T_RAMPON_IGN_ELEM',IZERO)
ENDIF

! arrays for veg volumes that have variables output to a file
...
ALLOCATE(VEG_TREE_NAME(N_TREES),STAT=IZERO)
CALL ChkMemErr('READ','VEG_TREE_NAME',IZERO)

!Arrays for reading in veg data from a binary file
ALLOCATE(N_TREE_IN_FILE(N_TREES),STAT=IZERO)
CALL ChkMemErr('READ','N_TREE_IN_FILE',IZERO) ; N_TREE_IN_FILE = 0

!Array for tree number that will be output to .prt5 file
ALLOCATE(N_TREE_FOR_PRT_FILE(N_TREES),STAT=IZERO)
CALL ChkMemErr('READ','N_TREE_FOR_PRT_FILE',IZERO) ; N_TREE_FOR_PRT_FILE = 0


!Arrays for CONE trees
!IF (N_CONE_TREES > 0) THEN
! ALLOCATE(X_TREE_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','X_TREE_CONE',IZERO)
! ALLOCATE(Y_TREE_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Y_TREE_CONE',IZERO)
! ALLOCATE(Z_TREE_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Z_TREE_CONE',IZERO)
! ALLOCATE(TREE_H_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','TREE_H_CONE',IZERO)
! ALLOCATE(CROWN_B_H_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_B_H_CONE',IZERO)
! ALLOCATE(CROWN_W_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_W_CONE',IZERO)
! ALLOCATE(IGN_ELEM_CONE(N_CONE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','IGN_ELEM_CONE',IZERO) ; IGN_ELEM_CONE = .FALSE.
! IF (N_CONE_IGNITORS > 0) THEN
!  ALLOCATE(TON_IGN_ELEM_CONE(N_CONE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TON_IGN_ELEM_CONE',IZERO)
!  ALLOCATE(TOFF_IGN_ELEM_CONE(N_CONE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TOFF_IGN_ELEM_CONE',IZERO)
!  ALLOCATE(T_RAMPOFF_IGN_ELEM_CONE(N_CONE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPOFF_IGN_ELEM_CONE',IZERO)
!  ALLOCATE(T_RAMPON_IGN_ELEM_CONE(N_CONE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPON_IGN_ELEM_CONE',IZERO)
! ENDIF
!ENDIF
!
!!Arrays for FRUSTUM trees
!IF (N_FRUSTUM_TREES > 0) THEN
! ALLOCATE(X_TREE_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','X_TREE_FRUSTUM',IZERO)
! ALLOCATE(Y_TREE_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Y_TREE_FRUSTUM',IZERO)
! ALLOCATE(Z_TREE_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Z_TREE_FRUSTUM',IZERO)
! ALLOCATE(TREE_H_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','TREE_H_FRUSTUM',IZERO)
! ALLOCATE(CROWN_B_H_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_B_H_FRUSTUM',IZERO)
! ALLOCATE(CROWN_W_TOP_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_W_TOP_FRUSTUM',IZERO)
! ALLOCATE(CROWN_W_BOTTOM_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_W_BOTTOM_FRUSTUM',IZERO)
! ALLOCATE(IGN_ELEM_FRUSTUM(N_FRUSTUM_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','IGN_ELEM_FRUSTUM',IZERO) ; IGN_ELEM_FRUSTUM = .FALSE.
! IF (N_FRUSTUM_IGNITORS > 0) THEN
!  ALLOCATE(TON_IGN_ELEM_FRUSTUM(N_FRUSTUM_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TON_IGN_ELEM_FRUSTUM',IZERO)
!  ALLOCATE(TOFF_IGN_ELEM_FRUSTUM(N_FRUSTUM_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TOFF_IGN_ELEM_FRUSTUM',IZERO)
!  ALLOCATE(T_RAMPOFF_IGN_ELEM_FRUSTUM(N_FRUSTUM_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPOFF_IGN_ELEM_FRUSTUM',IZERO)
!  ALLOCATE(T_RAMPON_IGN_ELEM_FRUSTUM(N_FRUSTUM_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPON_IGN_ELEM_FRUSTUM',IZERO)
! ENDIF
!ENDIF
!
!!Arrays for CYLINDER trees
!IF (N_CYLINDER_TREES > 0) THEN
! ALLOCATE(X_TREE_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','X_TREE_CYLINDER',IZERO)
! ALLOCATE(Y_TREE_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Y_TREE_CYLINDER',IZERO)
! ALLOCATE(Z_TREE_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Z_TREE_CYLINDER',IZERO)
! ALLOCATE(TREE_H_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','TREE_H_CYLINDER',IZERO)
! ALLOCATE(CROWN_B_H_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_B_H_CYLINDER',IZERO)
! ALLOCATE(CROWN_W_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_W_CYLINDER',IZERO)
! ALLOCATE(IGN_ELEM_CYLINDER(N_CYLINDER_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','IGN_ELEM_CYLINDER',IZERO) ; IGN_ELEM_CYLINDER = .FALSE.
! IF (N_CYLINDER_IGNITORS > 0) THEN
!  ALLOCATE(TON_IGN_ELEM_CYLINDER(N_CYLINDER_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TON_IGN_ELEM_CYLINDER',IZERO)
!  ALLOCATE(TOFF_IGN_ELEM_CYLINDER(N_CYLINDER_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TOFF_IGN_ELEM_CYLINDER',IZERO)
!  ALLOCATE(T_RAMPOFF_IGN_ELEM_CYLINDER(N_CYLINDER_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPOFF_IGN_ELEM_CYLINDER',IZERO)
!  ALLOCATE(T_RAMPON_IGN_ELEM_CYLINDER(N_CYLINDER_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPON_IGN_ELEM_CYLINDER',IZERO)
! ENDIF
!ENDIF
!
!
!Arrays for RECTANGLE trees
!IF (N_RECTANGLE_TREES > 0) THEN
! ALLOCATE(XS_RECT_VEG(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','XS_RECT_VEG',IZERO)
! ALLOCATE(XF_RECT_VEG(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','XF_RECT_VEG',IZERO)
! ALLOCATE(YS_RECT_VEG(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','YS_RECT_VEG',IZERO)
! ALLOCATE(YF_RECT_VEG(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','YF_RECT_VEG',IZERO)
! ALLOCATE(ZS_RECT_VEG(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','ZS_RECT_VEG',IZERO)
! ALLOCATE(ZF_RECT_VEG(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','ZF_RECT_VEG',IZERO)
! ALLOCATE(IGN_ELEM_RECT(N_RECTANGLE_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','IGN_ELEM_RECT',IZERO) ; IGN_ELEM_RECT = .FALSE.
! IF (N_RECTANGLE_IGNITORS > 0) THEN
!  ALLOCATE(TON_IGN_ELEM_RECT(N_RECTANGLE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TON_IGN_ELEM_RECT',IZERO)
!  ALLOCATE(TOFF_IGN_ELEM_RECT(N_RECTANGLE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TOFF_IGN_ELEM_RECT',IZERO)
!  ALLOCATE(T_RAMPOFF_IGN_ELEM_RECT(N_RECTANGLE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPOFF_IGN_ELEM_RECT',IZERO)
!  ALLOCATE(T_RAMPON_IGN_ELEM_RECT(N_RECTANGLE_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPON_IGN_ELEM_RECT',IZERO)
! ENDIF
!ENDIF
!
!!Arrays for RING trees
!IF (N_RING_TREES > 0) THEN
! ALLOCATE(RING_THICKNESS_VEG(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','RING_THICKNESS_VEG',IZERO)
! ALLOCATE(X_TREE_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','X_TREE_RING',IZERO)
! ALLOCATE(Y_TREE_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Y_TREE_RING',IZERO)
! ALLOCATE(Z_TREE_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','Z_TREE_RING',IZERO)
! ALLOCATE(TREE_H_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','TREE_H_RING',IZERO)
! ALLOCATE(CROWN_B_H_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_B_H_RING',IZERO)
! ALLOCATE(CROWN_W_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','CROWN_W_RING',IZERO)
! ALLOCATE(IGN_ELEM_RING(N_RING_TREES),STAT=IZERO)
! CALL ChkMemErr('READ','IGN_ELEM_RING',IZERO) ; IGN_ELEM_RING = .FALSE.
! IF (N_RING_IGNITORS > 0) THEN
!  ALLOCATE(TON_IGN_ELEM_RING(N_RING_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TON_IGN_ELEM_RING',IZERO)
!  ALLOCATE(TOFF_IGN_ELEM_RING(N_RING_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','TOFF_IGN_ELEM_RING',IZERO)
!  ALLOCATE(T_RAMPOFF_IGN_ELEM_RING(N_RING_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPOFF_IGN_ELEM_RING',IZERO)
!  ALLOCATE(T_RAMPON_IGN_ELEM_RING(N_RING_IGNITORS),STAT=IZERO)
!  CALL ChkMemErr('READ','T_RAMPON_IGN_ELEM_RING',IZERO)
! ENDIF
!ENDIF

!
! Read in TREE lines again to fill arrays
!
N_TREES_0           = N_TREES
N                   = 0
N_TREES_OUTPUT_DATA = 0
...
!
   ...
   IF (PART_ID  =='null') THEN 
         WRITE(MESSAGE,'(A,I6)') 'ERROR: Specify PART_ID in TREE namelist ',N
         CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (FUEL_GEOM=='null' .AND. .NOT. TREE_IN_FILE) THEN
         WRITE(MESSAGE,'(A,I6)') 'ERROR: Specify FUEL_GEOM in TREE namelist ',N
         CALL SHUTDOWN(MESSAGE)
   ENDIF
!
! Identify the number of the tree among all trees with OUTPUT_TREE = T that should have tree average stats output
   IF (OUTPUT_TREE .AND. LABEL /= 'null') THEN
    N_TREES_OUTPUT_DATA   = N_TREES_OUTPUT_DATA + 1
    N_TREE_OUT(N) = N_TREES_OUTPUT_DATA
   ENDIF

! Identify the number of the &TREE line(s) that will be read from a file
   IF (TREE_IN_FILE) THEN
    N_TREES_IN_FILE  = N_TREES_IN_FILE + 1
    N_TREE_IN_FILE(N) = N_TREES_IN_FILE
   ENDIF

! Identify the number of the &TREE line(s) that will be output to the .prt5 file, instead of TAG
   IF (OUTPUT_TREE) THEN
    N_TREE_FOR_PRT_FILE(NN) = NN
   ENDIF
...
!
   ...
   IGN_ELEMS(N)      = IGNITOR_ELEMENTS
!
! Specifiy veg geometry specific quantities
!
   IF (FUEL_GEOM=='CONE' .OR. FUEL_GEOM=='CYLINDER' .OR. FUEL_GEOM=='RING') THEN
     N_CFCR_TREES              = N_CFCR_TREES + 1
     TREE_CFCR_INDEX(N)        = N_CFCR_TREES
     CROWN_W(N_CFCR_TREES)     = CROWN_WIDTH
     CROWN_B_H(N_CFCR_TREES)   = CROWN_BASE_HEIGHT
     TREE_H(N_CFCR_TREES) = TREE_HEIGHT
     X_TREE(N_CFCR_TREES) = XYZ(1)
     Y_TREE(N_CFCR_TREES) = XYZ(2)
     Z_TREE(N_CFCR_TREES) = XYZ(3)
     IF (IGNITOR_ELEMENTS) THEN
      N_IGNITORS                      = N_IGNITORS + 1
      TREE_IGN_INDEX(N)               = N_IGNITORS
      TON_IGN_ELEMS(N_IGNITORS)       = TON_IGNITOR_ELEMENTS
      TOFF_IGN_ELEMS(N_IGNITORS)      = TOFF_IGNITOR_ELEMENTS
      T_RAMPON_IGN_ELEMS(N_IGNITORS)  = T_RAMPON_IGNITOR_ELEMENTS
      T_RAMPOFF_IGN_ELEMS(N_IGNITORS) = T_RAMPOFF_IGNITOR_ELEMENTS
     ENDIF
   ENDIF
!
   IF (FUEL_GEOM=='FRUSTUM') THEN
     ...
     Y_TREE(N_CFCR_TREES) = XYZ(2)
     Z_TREE(N_CFCR_TREES) = XYZ(3)
     N_FRUSTUM_TREES = N_FRUSTUM_TREES + 1
     TREE_FRUSTUM_INDEX(N)           = N_FRUSTUM_TREES
     CROWN_W_BOTTOM(N_FRUSTUM_TREES) = CROWN_WIDTH_BOTTOM
     CROWN_W_TOP(N_FRUSTUM_TREES)    = CROWN_WIDTH_TOP
     IF (IGNITOR_ELEMENTS) THEN
      N_IGNITORS                      = N_IGNITORS + 1
      TREE_IGN_INDEX(N)               = N_IGNITORS
      TON_IGN_ELEMS(N_IGNITORS)       = TON_IGNITOR_ELEMENTS
...
!
   IF (FUEL_GEOM=='RECTANGLE') THEN
     N_RECTANGLE_TREES  = N_RECTANGLE_TREES + 1
     TREE_RECT_INDEX(N) = N_RECTANGLE_TREES
     XS_RECT_VEG(N_RECTANGLE_TREES) = XB(1)
     XF_RECT_VEG(N_RECTANGLE_TREES) = XB(2)
     YS_RECT_VEG(N_RECTANGLE_TREES) = XB(3)
     YF_RECT_VEG(N_RECTANGLE_TREES) = XB(4)
     ZS_RECT_VEG(N_RECTANGLE_TREES) = XB(5)
     ZF_RECT_VEG(N_RECTANGLE_TREES) = XB(6)
     IF (IGNITOR_ELEMENTS) THEN
      N_IGNITORS                      = N_IGNITORS + 1
      TREE_IGN_INDEX(N)               = N_IGNITORS
      TON_IGN_ELEMS(N_IGNITORS)       = TON_IGNITOR_ELEMENTS
   ...
   ENDIF
!
   IF (FUEL_GEOM=='RING') THEN
     N_RING_TREES       = N_RING_TREES + 1
     TREE_RING_INDEX(N) = N_RING_TREES
     RING_THICKNESS_VEG(N_RING_TREES)  = RING_THICKNESS
!   IF (IGNITOR_ELEMENTS) THEN
!     N_IGNITORS                      = N_IGNITORS + 1
!     TREE_IGN_INDEX(N)               = N_IGNITORS
!     TON_IGN_ELEMS(N_IGNITORS)       = TON_IGNITOR_ELEMENTS
...
!    ENDIF
   ENDIF
!
! Assign particle class to each TREE
   DO IPC=1,N_PART
      PC=>PARTICLE_CLASS(IPC)
   ...
   PARTICLE_FILE=.TRUE.
!
! Assign character string for files containing tree averaged quantities for each TREE
   IF (LABEL == 'null') THEN
     VEG_LABELS(N) = 'no_tree_data_output'
   ELSE
     VEG_LABELS(N) = LABEL
   ENDIF
!
! Assign character string for tree name that will also be in binary file containing TREE
! data
   IF (TREE_NAME == 'null') THEN
     VEG_TREE_NAME(N) = 'no_tree_data_from_file'
   ELSE
     VEG_TREE_NAME(N) = TREE_NAME
   ENDIF
!
! Find meshes that require user specified ouput of vegetation quantities for the whole tree
   FIND_TREE_MESH_OUT: DO NM = 1, NMESHES
     IF (VEG_LABELS(N) == 'no_tree_data_output') CYCLE FIND_TREE_MESH_OUT
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BEAD_DENSITY,BEAD_DIAMETER,BEAD_EMISSIVITY,&
                BEAD_H_FIXED,BEAD_SPECIFIC_HEAT,BETA_C,BETA_E,CABLE_DIAMETER,CABLE_FAILURE_TEMPERATURE,CABLE_JACKET_THICKNESS,&
                CABLE_MASS_PER_LENGTH,CHARACTERISTIC_VELOCITY,CONDUIT_DIAMETER,CONDUIT_THICKNESS,C_FACTOR,DROPLET_VELOCITY,&
                PARTICLES_PER_SECOND,&
                PARTICLE_VELOCITY,FLOW_RAMP,FLOW_RATE,FLOW_TAU,GAUGE_TEMPERATURE,ID,INITIAL_TEMPERATURE,K_FACTOR,LENGTH,OFFSET,&
                OPERATING_PRESSURE,ORIFICE_DIAMETER,P0,PART_ID,PDPA_END,PDPA_HISTOGRAM,PDPA_HISTOGRAM_LIMITS,&
                PDPA_HISTOGRAM_NBINS,PDPA_INTEGRATE,PDPA_M,PDPA_N,PDPA_NORMALIZE,PDPA_RADIUS,&
...
!   PY%PARTICLE_INSERT_CLOCK    = T_BEGIN
   ...
   IF(PDPA_HISTOGRAM) THEN
      ...
      IF(PDPA_HISTOGRAM_NBINS<2) THEN
         ...
         CALL SHUTDOWN(MESSAGE)
      ENDIF
       IF(PDPA_HISTOGRAM_LIMITS(1) == PDPA_HISTOGRAM_LIMITS(2)) THEN
...
PROP_LOOP: DO N=0,N_PROP
   ...
   IF (PY%FLOW_RATE > 0._EB .OR. PY%OPERATING_PRESSURE > 0._EB) THEN
      DO NN=1,N_PART
         IF (TRIM(PARTICLE_CLASS(NN)%ID)==TRIM(PY%PART_ID)) THEN
            IF(PARTICLE_CLASS(NN)%DENSITY <0._EB) THEN
...
CHARACTER(25) :: COLOR
CHARACTER(25) :: VEG_DEGRADATION,VEG_LSET_SURFACE_FIRE_HEAD_ROS_MODEL,VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL, &
                 VEG_LSET_MODEL_FOR_PASSIVE_ROS
TYPE (PROPERTY_TYPE), POINTER :: PY=>NULL()
...
INTEGER ::  VEGETATION_LAYERS,VEG_LSET_UAVG_K
REAL(EB) :: VEG_DRAG_CONSTANT,VEG_ASH_FRACTION,VEG_CHAR_FRACTION,VEG_DENSITY,VEG_HEIGHT, &
            VEG_INITIAL_TEMP,VEG_LOAD,VEG_LSET_BURNER_TIME_OFF,VEG_LSET_BURNER_TIME_ON,VEG_LSET_IGNITE_TIME,VEG_LSET_QCON, &
            VEG_MOISTURE,VEG_SV,FIRELINE_MLR_MAX,VEGETATION_GROUND_TEMP,VEG_POSTFIRE_DRAG_FCTR
REAL(EB) :: VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
            VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION
REAL(EB) :: VEG_LSET_ROS_HEAD,VEG_LSET_ROS_FLANK,VEG_LSET_ROS_BACK, &
            VEG_LSET_WIND_EXP,VEG_LSET_BETA,VEG_LSET_FIREBASE_TIME,VEG_LSET_SURF_HEIGHT,VEG_LSET_SIGMA,VEG_LSET_ROTH_ZEROWINDSLOPE_ROS, &
            VEG_LSET_ROTHFM10_ZEROWINDSLOPE_ROS,VEG_LSET_PHIDEPTH, &
            VEG_LSET_HEAT_OF_COMBUSTION,VEG_LSET_SURF_LOAD,VEG_LSET_CHAR_FRACTION,VEG_LSET_SURF_EFFM, &
            VEG_LSET_CANOPY_BULK_DENSITY,VEG_LSET_CANOPY_FMC,VEG_LSET_CANOPY_BASE_HEIGHT,VEG_LSET_CANOPY_HEIGHT,VEG_LSET_FUEL_STRATA_GAP, &
            VEG_LSET_CRUZ_PROB_PASSIVE, &
            VEG_LSET_CRUZ_PROB_ACTIVE,VEG_LSET_CRUZ_PROB_CROWN,VEG_LSET_WAF_UNSHELTERED,VEG_LSET_WAF_SHELTERED,VEG_LSET_CROWNFIRE_ANGLE, &
            VEG_LSET_HRRPUA_MINIMUM_FRAC,VEG_LSET_UAVG_TIME
LOGICAL ::  VEG_LSET_HEADWIDTH_DEPENDENCE,VEGETATION,VEG_NO_BURN, &
            VEG_CHAR_OXIDATION,VEG_HCONV_CYLRE,VEG_HCONV_CYLLAM,VEG_HCONV_CYLMAX, &
            VEG_LEVEL_SET_SPREAD,VEG_LSET_BURNER,VEG_UNIT_DRAG_COEFF, &
            DEFAULT,EVAC_DEFAULT,VEG_LSET_ELLIPSE,VEG_LSET_TAN2,VEG_LSET_SURFACE_HRRPUA
...
NAMELIST /SURF/ ADIABATIC,BACKING,BOUNDARY_CONDITION_FILENAME,BURN_AWAY,CELL_SIZE_FACTOR,COLOR,CONVECTION_LENGTH_SCALE, &
                ...
                TMP_BACK,TMP_FRONT,TMP_INNER,TRANSPARENCY,VEGETATION, &
                VEG_CHAR_OXIDATION,VEG_DEGRADATION,VEG_HCONV_CYLRE,VEG_HCONV_CYLLAM,VEG_HCONV_CYLMAX,VEG_DRAG_CONSTANT,&
                VEG_ASH_FRACTION,VEG_CHAR_FRACTION,VEG_DENSITY,VEGETATION_GROUND_TEMP,VEG_HEIGHT,&
                VEG_INITIAL_TEMP,VEGETATION_LAYERS,VEG_LOAD,VEG_H_PYR, &
                VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
                VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION,VEG_UNIT_DRAG_COEFF, &
                VEG_LSET_BURNER,VEG_LSET_BURNER_TIME_OFF,VEG_LSET_BURNER_TIME_ON,VEG_LSET_IGNITE_TIME, &
                VEG_LSET_QCON,VEG_MOISTURE,VEG_NO_BURN,VEG_POSTFIRE_DRAG_FCTR,VEG_SV,VEG_LEVEL_SET_SPREAD, &
                VEG_LSET_ROS_BACK,VEG_LSET_ROS_FLANK,VEG_LSET_ROS_HEAD,VEG_LSET_WIND_EXP, &
                VEL,VEL_T,VOLUME_FLUX,WIDTH,XYZ,Z0,VEG_LSET_FIREBASE_TIME,VEG_LSET_SIGMA,VEG_LSET_SURF_HEIGHT,&
                VEG_LSET_BETA,VEG_LSET_ELLIPSE,VEG_LSET_TAN2,VEG_LSET_ROTH_ZEROWINDSLOPE_ROS,VEG_LSET_ROTHFM10_ZEROWINDSLOPE_ROS, &
                VEG_LSET_HEADWIDTH_DEPENDENCE, &
                VEG_LSET_PHIDEPTH,VEG_LSET_HEAT_OF_COMBUSTION,VEG_LSET_SURF_LOAD,VEG_LSET_CHAR_FRACTION,VEG_LSET_SURF_EFFM, &
                VEG_LSET_CANOPY_BULK_DENSITY,VEG_LSET_CANOPY_FMC,VEG_LSET_CANOPY_BASE_HEIGHT,VEG_LSET_CANOPY_HEIGHT, &
                VEG_LSET_FUEL_STRATA_GAP,VEG_LSET_CRUZ_PROB_PASSIVE, &
                VEG_LSET_CRUZ_PROB_ACTIVE,VEG_LSET_CRUZ_PROB_CROWN,VEG_LSET_SURFACE_FIRE_HEAD_ROS_MODEL,                 &
                VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL,VEG_LSET_MODEL_FOR_PASSIVE_ROS,VEG_LSET_WAF_UNSHELTERED, &
                VEG_LSET_WAF_SHELTERED,VEG_LSET_CROWNFIRE_ANGLE,VEG_LSET_HRRPUA_MINIMUM_FRAC,VEG_LSET_UAVG_K,VEG_LSET_UAVG_TIME, &
                VEG_LSET_SURFACE_HRRPUA
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   SF%VEG_LSET_ROTH_ZEROWINDSLOPE_ROS  = VEG_LSET_ROTH_ZEROWINDSLOPE_ROS !Rothermel no-wind, no-slope ROS 
   SF%VEG_LSET_ROTHFM10_ZEROWINDSLOPE_ROS  = VEG_LSET_ROTHFM10_ZEROWINDSLOPE_ROS !for fuel model 10
...
!                                             is used and burn time counter starts when phi >= -VEG_LSET_DEPTH
   ...
   SF%VEG_LSET_QCON                = -VEG_LSET_QCON*1000._EB !convert from kW/m^2 to W/m^2 and change sign
   SF%VEG_LSET_HEAT_OF_COMBUSTION  = VEG_LSET_HEAT_OF_COMBUSTION*1000._EB !J/kg
   SF%VEG_LSET_SURF_LOAD           = VEG_LSET_SURF_LOAD !kg/m^2 surface veg loading
   SF%VEG_LSET_CHAR_FRACTION       = VEG_LSET_CHAR_FRACTION
   SF%VEG_LSET_SURF_EFFM           = VEG_LSET_SURF_EFFM*100._EB !fine fuel moisture surface veg in %
   SF%VEG_LSET_CANOPY_FMC          = VEG_LSET_CANOPY_FMC*100._EB !canopy foliar moisture in %
   SF%VEG_LSET_CANOPY_BULK_DENSITY = VEG_LSET_CANOPY_BULK_DENSITY
   SF%VEG_LSET_CANOPY_BASE_HEIGHT  = VEG_LSET_CANOPY_BASE_HEIGHT 
   SF%VEG_LSET_CANOPY_HEIGHT       = VEG_LSET_CANOPY_HEIGHT 
   SF%VEG_LSET_FUEL_STRATA_GAP     = VEG_LSET_FUEL_STRATA_GAP  
   SF%VEG_LSET_CRUZ_PROB_PASSIVE   = VEG_LSET_CRUZ_PROB_PASSIVE
   SF%VEG_LSET_CRUZ_PROB_ACTIVE    = VEG_LSET_CRUZ_PROB_ACTIVE 
   SF%VEG_LSET_CRUZ_PROB_CROWN     = VEG_LSET_CRUZ_PROB_CROWN  
   SF%VEG_LSET_SURFACE_FIRE_HEAD_ROS_MODEL = VEG_LSET_SURFACE_FIRE_HEAD_ROS_MODEL
   SF%VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL   = VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL
   SF%VEG_LSET_MODEL_FOR_PASSIVE_ROS       = VEG_LSET_MODEL_FOR_PASSIVE_ROS
   SF%VEG_LSET_WAF_UNSHELTERED     = VEG_LSET_WAF_UNSHELTERED
   SF%VEG_LSET_WAF_SHELTERED       = VEG_LSET_WAF_SHELTERED
   SF%VEG_LSET_CROWNFIRE_ANGLE     = VEG_LSET_CROWNFIRE_ANGLE
   SF%VEG_LSET_HRRPUA_MINIMUM_FRAC = VEG_LSET_HRRPUA_MINIMUM_FRAC
   SF%VEG_LSET_UAVG_K              = VEG_LSET_UAVG_K
   SF%VEG_LSET_UAVG_TIME           = VEG_LSET_UAVG_TIME
   SF%VEG_LSET_SURFACE_HRRPUA      = VEG_LSET_SURFACE_HRRPUA !for direct combustion modeling instead of TE
   IF(VEG_LSET_SURFACE_HRRPUA) VEG_LEVEL_SET_SURFACE_HRRPUA = .TRUE. !for program flow
   SF%VEG_LSET_DRAG_INI = VEG_DRAG_CONSTANT*SF%VEG_LSET_BETA*SF%VEG_LSET_SIGMA*100._EB !x100 for sav in 1/m


   IF (VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL == 'SR') THEN
     SF%VEG_LSET_SR_CROWNROS_MODEL = .TRUE.
     VEG_LEVEL_SET_FM10_SPREADRATE = .TRUE. !efficiency would improve if this was a mesh variable
     VEG_LEVEL_SET_SR_CROWNFIRE_MODEL = .TRUE.
   ENDIF 

   IF (VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL == 'CRUZ') VEG_LEVEL_SET_CFIS_CROWNFIRE_MODEL=.TRUE.

   ! Boundary Vegetation specific

   SF%VEGETATION   = VEGETATION !T or F
   SF%VEG_NO_BURN  = VEG_NO_BURN
   IF(WIND_ONLY) SF%VEG_NO_BURN = .TRUE.
!  IF(SF%VEGETATION) ADIABATIC = .TRUE.
   SF%VEG_CHAR_FRACTION = VEG_CHAR_FRACTION
   SF%VEG_ASH_FRACTION  = VEG_ASH_FRACTION  !fraction of original dry fuel mass that can become ash
   SF%VEG_MOISTURE = VEG_MOISTURE
   SF%VEG_HEIGHT   = VEG_HEIGHT
   SF%VEG_INITIAL_TEMP = VEG_INITIAL_TEMP
   SF%VEG_GROUND_TEMP  = VEGETATION_GROUND_TEMP
   IF (VEGETATION_GROUND_TEMP /= -99._EB) SF%VEG_GROUND_ZERO_RAD = .FALSE.
   SF%VEG_LOAD     = VEG_LOAD
   SF%FIRELINE_MLR_MAX = FIRELINE_MLR_MAX
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   SF%VEG_PACKING  = VEG_LOAD/VEG_HEIGHT/VEG_DENSITY
   SF%VEG_SV  = VEG_SV
   ...
   SF%VEG_CHAR_OXIDATION   = VEG_CHAR_OXIDATION
   SF%VEG_HCONV_CYLRE  = VEG_HCONV_CYLRE
   SF%VEG_HCONV_CYLLAM = VEG_HCONV_CYLLAM
   SF%VEG_HCONV_CYLMAX = VEG_HCONV_CYLMAX   
   SF%VEG_POSTFIRE_DRAG_FCTR = VEG_POSTFIRE_DRAG_FCTR
   
   SF%VEG_CHAR_ENTHALPY_FRACTION = VEG_CHAR_ENTHALPY_FRACTION
   SF%VEG_H_H2O = VEG_H_H2O*1000._EB !J/kg/K
   SF%VEG_A_H2O = VEG_A_H2O
   SF%VEG_E_H2O = VEG_E_H2O
   SF%VEG_H_PYR = VEG_H_PYR*1000._EB !J/kg
   SF%VEG_A_PYR = VEG_A_PYR 
   SF%VEG_E_PYR = VEG_E_PYR 
   SF%VEG_H_CHAR = VEG_H_CHAR*1000._EB !J/kg
   SF%VEG_A_CHAR = VEG_A_CHAR
   SF%VEG_E_CHAR = VEG_E_CHAR
   SF%VEG_BETA_CHAR = VEG_BETA_CHAR 
   SF%VEG_NU_O2_CHAR = VEG_NU_O2_CHAR
   SF%VEG_UNIT_DRAG_COEFF = VEG_UNIT_DRAG_COEFF 
   ...
   ALLOCATE(SF%VEG_KGAS_L(SF%NVEG_L),STAT=IZERO)
   CALL ChkMemErr('READ_SURF','VEG_KGAS_L',IZERO)
...
! Boundary fuel vegetation model default values
VEGETATION                   = .FALSE.
VEG_NO_BURN                  = .FALSE.
VEG_DRAG_CONSTANT            = 1.0_EB
VEG_CHAR_FRACTION            = 0.20_EB
VEG_ASH_FRACTION             = 0.03_EB !fraction of original dry fuel mass that can become ash
VEG_DENSITY                  = 512._EB !kg/m^3
VEG_HEIGHT                   = 0.50_EB !m
VEG_INITIAL_TEMP             = TMPA - TMPM
VEGETATION_GROUND_TEMP       = -99._EB
VEG_LOAD                     = 0.30_EB !kg/m^2
FIRELINE_MLR_MAX             = 999. !kg/m/s w*R*(1-ChiChar) 
!SRF_VEG_DEHYDRATION_RATE_MAX = 999. !kg/m^2/s   
VEGETATION_LAYERS            = 0
VEG_MOISTURE                 = 0.06_EB
VEG_SV                       = 10000_EB !1/m
VEG_DEGRADATION              = 'LINEAR'
VEG_CHAR_OXIDATION           = .FALSE.
VEG_HCONV_CYLRE              = .FALSE. !cylindrical Re dependent convection heat transfer
VEG_HCONV_CYLLAM             = .TRUE. !cylindrical laminar convection heat transfer (Holman)
VEG_HCONV_CYLMAX             = .FALSE. !maxmium of cylindrical natural (Morgan) vs. forced (Hilpert) heat transfer
VEG_POSTFIRE_DRAG_FCTR       = 0.0_EB !Fraction of initial drag remaining after BF veg is burned away
...
VEG_LSET_BURNER_TIME_OFF     = 1.0E20_EB
VEG_LSET_BURNER_TIME_ON      = 1.0E20_EB
VEG_LSET_ROS_HEAD            = 0.0_EB
VEG_LSET_ROTH_ZEROWINDSLOPE_ROS = 0.0_EB !zero wind, zero slope ROS (m/s) from Rothermel 
VEG_LSET_ROTHFM10_ZEROWINDSLOPE_ROS = 0.0_EB !zero wind, zero slope ROS (m/s) from Rothermel Fuel Model 10 
...
VEG_LSET_TAN2                = .FALSE.
VEG_LSET_SURF_HEIGHT         = 0.5_EB
VEG_LSET_BETA                = 0.0012_EB
VEG_LSET_SIGMA               = 10000._EB !1/m
VEG_LSET_FIREBASE_TIME       = -1._EB
VEG_LSET_QCON                = 0.0_EB !kW/m^2
VEG_LSET_PHIDEPTH            = 0.5_EB 
VEG_LSET_HEADWIDTH_DEPENDENCE = .FALSE.
VEG_LSET_HEAT_OF_COMBUSTION  = 17700._EB !kJ/kg
VEG_LSET_SURF_LOAD           = 0.3_EB !kg/m^2
VEG_LSET_CHAR_FRACTION       = 0.2_EB
VEG_LSET_SURF_EFFM           = 0.1_EB
VEG_LSET_CANOPY_FMC          = 1.0_EB
VEG_LSET_CANOPY_BULK_DENSITY = 0.2_EB !kg/m^3
VEG_LSET_CANOPY_BASE_HEIGHT  = 0.0_EB !m
VEG_LSET_CANOPY_HEIGHT       = 0.0_EB !m
VEG_LSET_FUEL_STRATA_GAP     = 4.3_EB !m Fuel strata gap, distance from top of surface veg to canopy base
VEG_LSET_CRUZ_PROB_PASSIVE   = 999._EB
VEG_LSET_CRUZ_PROB_ACTIVE    = 999._EB
...
VEG_LSET_HRRPUA_MINIMUM_FRAC = 0.0_EB
VEG_LSET_UAVG_K              = -1
VEG_LSET_UAVG_TIME           = 10.0_EB !default 10s time interval for time averaging when ROSvsU model is used
VEG_LSET_SURFACE_HRRPUA      = .FALSE. !for direct combustion modeling, instead of TE
   ...
   DO IPC=1,N_PART
      ...
      IF (PC%SURF_ID==SF%ID) THEN
         ...
         IF (.NOT.SF%THERMALLY_THICK .AND. SF%RADIUS<0._EB) THEN
            ...
            CALL SHUTDOWN(MESSAGE)
         ENDIF
         IF (SF%GEOMETRY==SURF_CYLINDRICAL .AND. SF%LENGTH<0._EB) THEN
...
INTEGER :: IPC,N,NN,NNN,NL
REAL(EB) :: ADJUSTED_LAYER_DENSITY
INTEGER  :: IVEG_L,IIVEG_L,I_FUEL
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   IF (VEG_LEVEL_SET_COUPLED .AND. SF%HRRPUA > 0.0_EB) THEN !For Level Set mimic of fixed heat source
    IF (RN%HEAT_OF_COMBUSTION == 0.0_EB) BURNING = .FALSE.
   ENDIF
   ...
   !Level set using combustion model (i.e., not using thermal elements) for HRR

   IF (SF%VEG_LSET_SURFACE_HRRPUA) SF%SPECIES_BC_INDEX = SPECIFIED_MASS_FLUX
...
USE RADCONS
NAMELIST /RADI/ ANGLE_INCREMENT,CH4_BANDS,KAPPA0,NUMBER_RADIATION_ANGLES,NMIEANG,PATH_LENGTH,RADCAL_FUEL,&
...
REAL(EB) :: MINIMUM_DENSITY,MAXIMUM_DENSITY,MINIMUM_TEMPERATURE,MAXIMUM_TEMPERATURE
...
NAMELIST /RAMP/ DEVC_ID,CTRL_ID,F,FYI,ID,NUMBER_INTERPOLATION_POINTS,T,X
...
NAMELIST /VENT/ COLOR,CTRL_ID,DEVC_ID,DYNAMIC_PRESSURE,EVACUATION,FYI,ID,IOR,L_EDDY,L_EDDY_IJ,MB,MESH_ID,N_EDDY,OUTLINE,&
                PBX,PBY,PBZ,PRESSURE_RAMP, REYNOLDS_STRESS,RGB,SPREAD_RATE,SURF_ID,TEXTURE_ORIGIN,TMP_EXTERIOR,TRANSPARENCY,&
                VEL_RMS,UVW,XB,XYZ
...
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(1:MAX_SPECIES),RR_SUM,ZZ_GET(0:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
            MASS_PER_TIME,DT_INSERT,XYZ(3),UVW(3),DIAMETER,HRRPUV
INTEGER  :: N,NN,NNN,II,JJ,KK,NS,NS2,NUMBER_INITIAL_PARTICLES,N_INIT_NEW,N_INIT_READ
LOGICAL  :: UNIFORM = .FALSE.
CHARACTER(30) :: CTRL_ID,DEVC_ID,PART_ID,SHAPE,MULT_ID,ORIGIN='CENTER',PROF_ID,SPEC_ID(1:MAX_SPECIES)
TYPE(INITIALIZATION_TYPE), POINTER :: IN=>NULL()
TYPE(MULTIPLIER_TYPE), POINTER :: MR=>NULL()
NAMELIST /INIT/ CTRL_ID,DENSITY,DEVC_ID,DIAMETER,DT_INSERT,HRRPUV,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,&
                NUMBER_INITIAL_PARTICLES,ORIGIN,PART_ID,PROF_ID,SHAPE,SPEC_ID,TEMPERATURE,UNIFORM,UVW,XB,XYZ, &
                INIT_PLE,INIT_ATMOSPHERIC,INIT_Z0
         ...
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IF (PART_ID/='null') THEN
               ...
               IF (IN%PART_INDEX<1) THEN
                  ...
                  CALL SHUTDOWN(MESSAGE)
               ENDIF
               IN%NUMBER_INITIAL_PARTICLES = NUMBER_INITIAL_PARTICLES*PARTICLE_CLASS(IN%PART_INDEX)%N_SPLIT
            ...
            ! Special case: POINTWISE_PARTICLE_INIT

            POINTWISE_PARTICLE_IF: IF (ANY(XYZ>-1000._EB)) THEN
               IF (NUMBER_INITIAL_PARTICLES>1) THEN
                  WRITE(MESSAGE,'(A)') 'ERROR: NUMBER_INITIAL_PARTICLES=1 for pointwise insert'
                  CALL SHUTDOWN(MESSAGE)
               ENDIF
               IN%SHAPE = 'POINT'
               IN%NUMBER_INITIAL_PARTICLES=1     ! one PARTICLE per INIT line
               IN%POINTWISE_PARTICLE_INIT=.TRUE. ! flag for insert loop
               IN%X0 = XYZ(1) + MR%DX0
               IN%Y0 = XYZ(2) + MR%DY0
               IN%Z0 = XYZ(3) + MR%DZ0
               !IN%U0 = UVW(1)
...
INTEGER  :: NN,NM,MESH_NUMBER,N_DEVC_READ,IOR,TRIP_DIRECTION,VELO_INDEX,POINTS,I_POINT,VIRTUAL_INDEX
REAL(EB) :: DEPTH,ORIENTATION(3),ROTATION,SETPOINT,FLOWRATE,BYPASS_FLOWRATE,DELAY,XYZ(3),CONVERSION_FACTOR,SMOOTHING_FACTOR
CHARACTER(30) :: QUANTITY,PROP_ID,CTRL_ID,DEVC_ID,SURF_ID,STATISTICS,PART_ID,MATL_ID,SPEC_ID,UNITS,DUCT_ID,NODE_ID(2),X_ID,Y_ID,Z_ID
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                HIDE_COORDINATES,ID,INITIAL_STATE,IOR,LATCH,MATL_ID,NODE_ID,ORIENTATION,OUTPUT,PART_ID,POINTS,PROP_ID,QUANTITY,&
                RELATIVE,ROTATION,SETPOINT,SMOOTHING_FACTOR,SPEC_ID,STATISTICS,SURF_ID,TIME_AVERAGED,TRIP_DIRECTION,UNITS,&
                VELO_INDEX,VIRTUAL_INDEX,XB,XYZ,X_ID,Y_ID,Z_ID
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      DV%SURF_INDEX       = 0
      DV%UNITS            = UNITS
      DV%VIRTUAL_INDEX    = VIRTUAL_INDEX
...
NAMELIST /CTRL/ CYCLES,CYCLE_TIME,DELAY,EVACUATION,FUNCTION_TYPE,ID,INITIAL_STATE,INPUT_ID,LATCH,N,ON_BOUND,RAMP_ID,SETPOINT
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%QUANTITY /= 'null') THEN
      ...
      IF (QUANTITY_INDEX<0 .AND. DV%IOR==0 .AND. DV%STATISTICS=='null' .AND. DV%VIRTUAL_INDEX<1) THEN
...
NAMELIST /ISOF/ AUTO_HRRPUV_VALUE,COLOR_QUANTITY,COLOR_SPEC_ID,FYI,QUANTITY,REDUCE_TRIANGLES,SPEC_ID,VALUE,VELO_INDEX
...
READ_ISOF_LOOP: DO N=1,N_ISOF
   ...
   IF (COLOR_QUANTITY/='null') &
      CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL2,IS%SMOKEVIEW_BAR_LABEL2,IS%INDEX2,I_DUM, &
                              IS%Y_INDEX2,IS%Z_INDEX2,I_DUM,I_DUM,I_DUM,'ISOF', &
...
LOGICAL :: CELL_CENTERED
CHARACTER(30) :: QUANTITY,PROP_ID,SPEC_ID,PART_ID
NAMELIST /BNDE/ CELL_CENTERED,FYI,PART_ID,PROP_ID,QUANTITY,RECOUNT_DRIP,SPEC_ID

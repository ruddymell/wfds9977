...
PRIVATE
INTEGER IZERO
CHARACTER(255), PARAMETER :: initid='$Id: init.f90 9965 2012-02-02 21:08:17Z mcgratta $'
CHARACTER(255), PARAMETER :: initrev='$Revision: 9965 $'
CHARACTER(255), PARAMETER :: initdate='$Date: 2012-02-02 13:08:17 -0800 (Thu, 02 Feb 2012) $'

PUBLIC INITIALIZE_MESH_VARIABLES,INITIALIZE_GLOBAL_VARIABLES,OPEN_AND_CLOSE,INITIAL_NOISE,UVW_INIT, &
       INITIAL_ATMOSPHERIC_PROFILE,GET_REV_init
...
IF (VEG_LEVEL_SET) THEN
  ALLOCATE(M%LSET_PHI(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','LSET_PHI',IZERO) ; M%LSET_PHI = -1._EB
  ...
  ALLOCATE(M%ROS_HEAD_SA(0:IBP1,0:JBP1),STAT=IZERO)   ; CALL ChkMemErr('INIT','ROS_HEAD_SA',IZERO) ; M%ROS_HEAD_SA= 0.0_EB
  ALLOCATE(M%ROS_HEAD_SURF(0:IBP1,0:JBP1),STAT=IZERO)   ; CALL ChkMemErr('INIT','ROS_HEAD_SURF',IZERO) ; M%ROS_HEAD_SURF= 0.0_EB
  ...
  ALLOCATE(M%WIND_EXP(0:IBP1,0:JBP1),STAT=IZERO)   ; CALL ChkMemErr('INIT','WIND_EXP',IZERO) ; M%WIND_EXP= 1.0_EB
  ALLOCATE(M%FLANKFIRE_LIFETIME(0:IBP1,0:JBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','FLANKFIRE_LIFETIME',IZERO)
  M%FLANKFIRE_LIFETIME = 0.0_EB

  ALLOCATE(M%PHI_LS(0:IBP1,0:JBP1))  ; CALL ChkMemErr('INIT','PHI_LS',IZERO) ;         M%PHI_LS = -1._EB
  ALLOCATE(M%PHI0_LS(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','PHI0_LS',IZERO) ;        M%PHI0_LS = -1._EB
  ALLOCATE(M%PHI1_LS(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','PHI1_LS',IZERO) ;        M%PHI1_LS = -1._EB
  ALLOCATE(M%PHI_TEMP_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','PHI_TEMP_LS',IZERO) ; M%PHI_TEMP_LS = -1._EB

  ALLOCATE(M%FLUX_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','FLUX_LS',IZERO) ;   M%FLUX_LS  = 0.0_EB
  ALLOCATE(M%FLUX0_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','FLUX0_LS',IZERO) ; M%FLUX0_LS = 0.0_EB
  ...
  ALLOCATE(M%SR_X_RSA_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','SR_X_RSA_LS',IZERO)   ; M%SR_X_RSA_LS  = 0.0_EB
  ALLOCATE(M%SR_Y_RSA_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','SR_Y_RSA_LS',IZERO)   ; M%SR_Y_RSA_LS  = 0.0_EB
  ALLOCATE(M%SR_X_SURF_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','SR_X_SURF_LS',IZERO)   ; M%SR_X_SURF_LS  = 0.0_EB
  ALLOCATE(M%SR_Y_SURF_LS(0:IBP1,0:JBP1)); CALL ChkMemErr('INIT','SR_Y_SURF_LS',IZERO)   ; M%SR_Y_SURF_LS  = 0.0_EB
  ...
  ALLOCATE(M%U_LS_AVG(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','U_LS_AVG',IZERO) ; M%U_LS_AVG = 0._EB
  ALLOCATE(M%V_LS_AVG(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','V_LS_AVG',IZERO) ; M%V_LS_AVG = 0._EB
  ALLOCATE(M%U_LS_SUM(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','U_LS_SUM',IZERO) ; M%U_LS_SUM = 0._EB
  ALLOCATE(M%V_LS_SUM(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','V_LS_SUM',IZERO) ; M%V_LS_SUM = 0._EB

  ALLOCATE(M%BURN_TIME_LS(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','BURN_TIME_LS',IZERO) ; M%BURN_TIME_LS = 0._EB
  ALLOCATE(M%MASSPUA_CANOPY_CONSUMED(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','MASSPUA_CANOPY_CONSUMED',IZERO)
           M%MASSPUA_CANOPY_CONSUMED= 0._EB

!--- Slopes and gradients
  ALLOCATE(M%DZTDX(0:IBP1,0:JBP1))  ; CALL ChkMemErr('INIT','DZTDX',IZERO)  ; M%DZTDX= 0._EB
  ALLOCATE(M%DZTDY(0:IBP1,0:JBP1))  ; CALL ChkMemErr('INIT','DZTDY',IZERO)  ; M%DZTDY= 0._EB
  ALLOCATE(M%MAG_ZT(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','MAG_ZT',IZERO) ; M%MAG_ZT= 0._EB
  ALLOCATE(M%ASPECT(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','ASPECT',IZERO) ; M%ASPECT= 0._EB
...
!-- Arrays for ouputs
  ...
  ALLOCATE(M%TOA(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','TOA',IZERO) ; M%TOA = -1.0
  ALLOCATE(M%RINI_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','RINI_OUT',IZERO) ; M%RINI_OUT = 0.0
  ALLOCATE(M%ROS_FINAL_MAG_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS_FINAL_MAG_OUT',IZERO) ; M%ROS_FINAL_MAG_OUT = 0.0
  ALLOCATE(M%ROS_X_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS_X_OUT',IZERO) ; M%ROS_X_OUT = 0.0
  ALLOCATE(M%ROS_Y_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS_Y_OUT',IZERO) ; M%ROS_Y_OUT =  0.0
  ALLOCATE(M%ROS_SURF_X_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS_SURF_X_OUT',IZERO) ; M%ROS_SURF_X_OUT = 0.0
  ALLOCATE(M%ROS_SURF_Y_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS_SURF_Y_OUT',IZERO) ; M%ROS_SURF_Y_OUT =  0.0
  ALLOCATE(M%ROS10_X_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS10_X_OUT',IZERO) ; M%ROS10_X_OUT = 0.0
  ALLOCATE(M%ROS10_Y_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','ROS10_Y_OUT',IZERO) ; M%ROS10_Y_OUT =  0.0
  ALLOCATE(M%RSA_X_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','RSA_X_OUT',IZERO) ; M%RSA_X_OUT = 0.0
  ALLOCATE(M%RSA_Y_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','RSA_Y_OUT',IZERO) ; M%RSA_Y_OUT =  0.0
  ALLOCATE(M%FLI_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','FLI_OUT',IZERO) ; M%FLI_OUT = -1.0
  ALLOCATE(M%HRRPUA_OUT(0:IBAR,0:JBAR)) ; CALL ChkMemErr('INIT','HRRPUA_OUT',IZERO) ; M%HRRPUA_OUT= 0.0
...
!-- Arrays for implementing Scott and Reinhardt crown fire model
  ...
  ALLOCATE(M%ROS_FINAL_MAG(0:IBP1,0:JBP1))  ; CALL ChkMemErr('INIT','ROS_FINAL_MAG',IZERO)  ; M%ROS_FINAL_MAG= 0.0_EB
  ...
  ALLOCATE(M%FLAG_MODEL_FOR_PASSIVE_ROS(0:IBP1,0:JBP1)) ; CALL ChkMemErr('INIT','FLAG_MODEL_FOR_PASSIVE_ROS',IZERO)
                                                          M%FLAG_MODEL_FOR_PASSIVE_ROS = 0
...
!-- Arrays with names of files and LU for output files and HRRPUA input 
  ALLOCATE(M%FN_SLCF_LS(13)) ; CALL ChkMemErr('INIT','FN_SLCF_LS',IZERO)
  ALLOCATE(M%LU_SLCF_LS(13)) ; CALL ChkMemErr('INIT','LU_SLCF_LS',IZERO)
...
PROCESS_VENT: IF (I_VENT>0) THEN
   ...
   IF (SURFACE(SURF_INDEXX)%PROFILE==ATMOSPHERIC) THEN
      ...
      IF (U_Z0_UATM /= 0.0_EB) WC%UW0 = 0.0_EB
...
SUBROUTINE INITIAL_ATMOSPHERIC_PROFILE(NM)
USE GLOBAL_CONSTANTS

! Initialize wind with atmospheric profile throughout the computational domain
! Currently implemented for flow in the x direction only and for level ground
 
INTEGER  :: I,II,IOR,IW,J,JJ,K,KK
INTEGER, INTENT(IN) :: NM
TYPE(WALL_TYPE), POINTER :: WC=>NULL()
...
DO K=1,KBP1
   DO J=1,JBP1
      DO I=1,IBP1
         ...
         U(I,J,K)   = U_Z0_UATM*((M%ZC(K)-GROUND_LEVEL)/Z0_UATM)**PLE_UATM
      ENDDO 
   ENDDO
ENDDO
...
END SUBROUTINE INITIAL_ATMOSPHERIC_PROFILE
...
!SUBROUTINE UVW_INIT(NM,FN_UVW) !replaced with routine abouve from FDS6.2
!
!! Read UVW file
!

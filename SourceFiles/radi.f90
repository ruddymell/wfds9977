...
CHARACTER(255), PARAMETER :: radiid='$Id: radi.f90 9946 2012-01-30 22:31:01Z mcgratta $'
CHARACTER(255), PARAMETER :: radirev='$Revision: 9946 $'
CHARACTER(255), PARAMETER :: radidate='$Date: 2012-01-30 14:31:01 -0800 (Mon, 30 Jan 2012) $'
...
PARTICLES: IF (N_EVAP_INDICES>0) THEN
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   IF_PARTICLES_INCLUDED: IF (NLP>0 .AND. N_EVAP_INDICES>0) THEN
      IF (NUMBER_SPECTRAL_BANDS==1) THEN
         BBF = 1._EB
      ELSE
   ...
   IF_VEG_INCLUDED: IF (NLP>0 .AND. TREE_MESH(NM)) THEN
      VEG_ELEM_LOOP: DO ID = 1,NLP
         IPC = PARTICLE(ID)%CLASS
         IF (.NOT. PARTICLE_CLASS(IPC)%TREE) CYCLE VEG_ELEM_LOOP
         LP=>PARTICLE(ID)
         CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XID,YJD,ZKD,IID,JJD,KKD)
         KAPPAW(IID,JJD,KKD) = KAPPAW(IID,JJD,KKD) + PARTICLE(ID)%VEG_KAPPA
         KFST4W(IID,JJD,KKD) = KFST4W(IID,JJD,KKD) + PARTICLE(ID)%VEG_EMISS
      ENDDO VEG_ELEM_LOOP
      QR_W = 0._EB
      ...
      DO ID = 1,NLP
         ...
         SELECT CASE(SF%GEOMETRY)
            ...
            CASE(SURF_CYLINDRICAL)
               SURFACE_AREA = SF%LENGTH*TWOPI*LP%R
            CASE(SURF_SPHERICAL)
               SURFACE_AREA = 4._EB*PI*LP%R**2
         END SELECT
         KAPPA_PART = 0.25_EB*LP%PWT*SURFACE_AREA*RDX(IID)*RRN(IID)*RDY(JJD)*RDZ(KKD)
         KAPPAW(IID,JJD,KKD) = KAPPAW(IID,JJD,KKD) + KAPPA_PART
         KFST4W(IID,JJD,KKD) = KFST4W(IID,JJD,KKD) + FOUR_SIGMA*KAPPA_PART*LP%TMP**4
      ENDDO
      QR_W = 0._EB
   ENDIF
...
IF (.NOT. WIDE_BAND_MODEL) THEN
   ...
   IF_VEG_INCLUDED2: IF (NLP>0 .AND. TREE_MESH(NM)) THEN
      VEG_ELEM_LOOP2: DO ID = 1,NLP
         ...
         PARTICLE(ID)%VEG_DIVQR = 0.0_EB
         CALL GET_IJK(PARTICLE(ID)%X,PARTICLE(ID)%Y,PARTICLE(ID)%Z,NM,XID,YJD,ZKD,IID,JJD,KKD)
         PARTICLE(ID)%VEG_DIVQR = PARTICLE(ID)%VEG_KAPPA*UII(IID,JJD,KKD) - PARTICLE(ID)%VEG_EMISS

...
PRIVATE
CHARACTER(255), PARAMETER :: presid='$Id: pres.f90 9948 2012-01-31 15:46:25Z mcgratta $'
CHARACTER(255), PARAMETER :: presrev='$Revision: 9948 $'
CHARACTER(255), PARAMETER :: presdate='$Date: 2012-01-31 07:46:25 -0800 (Tue, 31 Jan 2012) $'
...
CHECK_WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(ABS(IOR))
      CASE(1)
         ...
         IF (IOR>0) WALL_WORK1(IW) = -0.25_EB*VELOCITY_ERROR*DX(II)/DT
         IF (IOR<0) WALL_WORK1(IW) =  0.25_EB*VELOCITY_ERROR*DX(II)/DT
      CASE(2)
         VELOCITY_ERROR = V_NEW - V_NEW_OTHER
         IF (IOR>0) WALL_WORK1(IW) = -0.25_EB*VELOCITY_ERROR*DY(JJ)/DT
         IF (IOR<0) WALL_WORK1(IW) =  0.25_EB*VELOCITY_ERROR*DY(JJ)/DT
      CASE(3)
         VELOCITY_ERROR = W_NEW - W_NEW_OTHER
         IF (IOR>0) WALL_WORK1(IW) = -0.25_EB*VELOCITY_ERROR*DZ(KK)/DT
         IF (IOR<0) WALL_WORK1(IW) =  0.25_EB*VELOCITY_ERROR*DZ(KK)/DT
   END SELECT
...
USE GLOBAL_CONSTANTS, ONLY: PREDICTOR,CORRECTOR,NMESHES
REAL(EB), INTENT(IN) :: B(:)
INTEGER :: NM,NOM
...
IF (PREDICTOR) THEN
   H  = H  + B(NM)
   DO NOM=1,NMESHES
      IF (OMESH(NOM)%NIC_S>0) OMESH(NOM)%H = OMESH(NOM)%H + B(NOM)
   ENDDO
ENDIF
...
IF (CORRECTOR) THEN
   HS = HS + B(NM)
   DO NOM=1,NMESHES
      IF (OMESH(NOM)%NIC_S>0) OMESH(NOM)%HS = OMESH(NOM)%HS + B(NOM)
   ENDDO
ENDIF

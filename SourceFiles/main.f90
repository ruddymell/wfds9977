...
ALLOCATE(REAL_BUFFER_4(N_TREES_OUTPUT_DATA,11,NMESHES))
...
DO NM=1,NMESHES
   ...
   IF (VEG_LEVEL_SET_UNCOUPLED .OR. VEG_LEVEL_SET_COUPLED) CALL INITIALIZE_LEVEL_SET_FIREFRONT(NM)
ENDDO

! Create or read in binary file containing raised veg information
IF (MYID==0) CALL CREATE_RAISED_VEG_FILE
DO NM=1,NMESHES
 IF (PROCESS(NM)/=MYID .OR. VEG_INPUT_FILENAME == 'null') CYCLE
 OPEN(UNIT=9137,FILE=VEG_INPUT_FILENAME,FORM='UNFORMATTED',STATUS='OLD') 
 CALL INITIALIZE_RAISED_VEG_FROM_FILE(NM)
ENDDO
...
MAIN_LOOP: DO  
   ...
   ! Determine time step
!print*,'main: synchronize, dt_sync',synchronize,dt_sync 
   IF (SYNCHRONIZE) THEN
      DO NM=1,NMESHES
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (VEG_LEVEL_SET) THEN
    ALLOCATE(M%OMESH(NOM)%LSET_PHI(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX))
    M%OMESH(NOM)%LSET_PHI = -1._EB
   ENDIF
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZATION_IF: IF (CODE==0) THEN
      ...
      IF (M3%NIC_S>0) THEN
         ...
         IF(.NOT. VEG_LEVEL_SET) ALLOCATE(M3%REAL_RECV_PKG3(IJK_SIZE*(4+N_TRACKED_SPECIES)))
         IF(      VEG_LEVEL_SET) ALLOCATE(M3%REAL_RECV_PKG3(IJK_SIZE*(4+N_TRACKED_SPECIES+1)))
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZE_SEND_IF: IF (CODE==0) THEN
      ...
      IF (M3%NIC_R>0 .AND. RNODE/=SNODE) THEN
         ...
         IF (.NOT. VEG_LEVEL_SET) ALLOCATE(M3%REAL_SEND_PKG3(IJK_SIZE*(4+N_TRACKED_SPECIES)))
         IF (      VEG_LEVEL_SET) ALLOCATE(M3%REAL_SEND_PKG3(IJK_SIZE*(4+N_TRACKED_SPECIES+1)))
   ...
   IF (CODE==4 .AND. M3%NIC_R>0) THEN
      IF (RNODE/=SNODE) THEN
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  LL = LL+4+N_TRACKED_SPECIES
                  IF (VEG_LEVEL_SET) THEN
                    M3%REAL_SEND_PKG3(LL+1) = M%LSET_PHI(II,JJ,KK)
                    LL = LL + 1
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         IF (.NOT. VEG_LEVEL_SET) CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),IJK_SIZE*(4+N_TRACKED_SPECIES),MPI_DOUBLE_PRECISION, &
                        SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         IF (      VEG_LEVEL_SET) CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),IJK_SIZE*(4+N_TRACKED_SPECIES+1),MPI_DOUBLE_PRECISION, &
                        SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
         M2=>MESHES(NOM)%OMESH(NM)
         ...
         IF (VEG_LEVEL_SET) M2%LSET_PHI(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX)= M%LSET_PHI(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX)
      ENDIF
   ENDIF
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               LL = LL+4+N_TRACKED_SPECIES
               IF (VEG_LEVEL_SET) THEN
                 M2%LSET_PHI(II,JJ,KK)= M2%REAL_RECV_PKG3(LL+1)
                 LL = LL + 1
               ENDIF
            ENDDO
         ENDDO
...
IF (N_TREES_OUTPUT_DATA>0 .AND. T>VEG_CLOCK) THEN
   COUNTS_TREE(:) = COUNTS(:)*N_TREES_OUTPUT_DATA*11
   DISPLS_TREE(:) = DISPLS(:)*N_TREES_OUTPUT_DATA*11
   IF (USE_MPI) THEN

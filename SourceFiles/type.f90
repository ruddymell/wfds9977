...
IMPLICIT NONE
CHARACTER(255), PARAMETER :: typeid='$Id: type.f90 9814 2012-01-11 17:49:04Z randy.mcdermott $'
CHARACTER(255), PARAMETER :: typerev='$Revision: 9814 $'
CHARACTER(255), PARAMETER :: typedate='$Date: 2012-01-11 09:49:04 -0800 (Wed, 11 Jan 2012) $'
...
TYPE PARTICLE_CLASS_TYPE
   ...
   REAL(EB) :: DENSITY,TMP_V=-1._EB,TMP_MELT=-1._EB,FTPR,HEAT_OF_COMBUSTION,ADJUST_EVAPORATION, &
               ...
               VEG_SV,VEG_MOISTURE,VEG_CHAR_FRACTION,VEG_ASH_FRACTION,VEG_DRAG_CONSTANT,VEG_BULK_DENSITY, &
               VEG_DENSITY,VEG_BURNING_RATE_MAX,VEG_CHAROX_RATE_MAX,VEG_DEHYDRATION_RATE_MAX,VEG_INITIAL_TEMPERATURE, &
               VEG_FUEL_MPV_MIN,VEG_MOIST_MPV_MIN,VEG_ASH_MPV_MAX,USER_DRAG_COEFFICIENT, &
               SURFACE_TENSION,BREAKUP_CHILD_DIAMETER,BREAKUP_CHILD_GAMMA,BREAKUP_CHILD_SIGMA,DENSE_VOLUME_FRACTION, &
               REAL_REFRACTIVE_INDEX,COMPLEX_REFRACTIVE_INDEX,TOL_INT,VEG_CHAR_MPV_MIN
   REAL(EB) :: VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
               VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION,TE_BURNTIME
   ...
   INTEGER :: SAMPLING,N_QUANTITIES,QUANTITIES_INDEX(10),EVAP_INDEX=0,RGB(3),RADIATIVE_PROPERTY_INDEX=0, &
              SURF_INDEX=-1,DRAG_LAW=1,DEVC_INDEX=0,CTRL_INDEX=0,PROP_INDEX=-1,N_SPLIT=1,SPLIT_PART_INDEX=0,Z_INDEX=-1,&
              Y_INDEX=-1,CNF_RAMP_INDEX=-1,CHILD_CNF_RAMP_INDEX=-1
   ...
   LOGICAL :: STATIC=.FALSE.,WATER=.FALSE.,FUEL=.FALSE.,MASSLESS,TREE=.FALSE.,MONODISPERSE=.FALSE.,EVAPORATE=.TRUE., &
              BREAKUP=.FALSE.,CHECK_DISTRIBUTION=.FALSE.
   LOGICAL VEG_REMOVE_CHARRED,VEG_STEM,VEG_CHAR_OXIDATION,VEG_HCONV_CYLLAM,VEG_KEEP_FOR_SMV
END TYPE PARTICLE_CLASS_TYPE

TYPE (PARTICLE_CLASS_TYPE), DIMENSION(:), ALLOCATABLE, TARGET :: PARTICLE_CLASS

TYPE PARTICLE_TYPE
   REAL(EB) :: X,Y,Z,TMP,U,V,W,R,PWT,ACCEL_X,ACCEL_Y,ACCEL_Z,T,RE=0._EB
   REAL(EB) :: VEG_KAPPA,VEG_EMISS,VEG_DIVQC,VEG_DIVQR,VEG_PACKING_RATIO,VEG_FUEL_MASS,VEG_MOIST_MASS,VEG_IGN_TON, &
               VEG_IGN_TOFF,VEG_IGN_TRAMPON,VEG_IGN_TRAMPOFF,VEG_CHAR_MASS,VEG_ASH_MASS,VEG_SV,VEG_MLR,&
               VEG_VOLFRACTION,LSET_HRRPUV,VEG_Q_CHAROX
   LOGICAL  :: IGNITOR,VEG_IGNITED,LSET_THERMAL_ELEMENT=.FALSE.
   INTEGER  :: IOR,CLASS,TAG,WALL_INDEX=0,VEG_N_TREE_OUTPUT,VEG_N_TREE_PRT_OUTPUT,SPLIT_IOR,SURF_INDEX=0
   LOGICAL  :: SHOW,SPLAT=.FALSE.
!! Shift to use 1D structure
!   REAL(EB), POINTER, DIMENSION(:) :: TMP_S, LAYER_THICKNESS,X_S
!   REAL(EB), POINTER, DIMENSION(:,:) :: RHO_S
!   INTEGER, POINTER, DIMENSION(:) :: N_LAYER_CELLS
!   REAL(EB), POINTER, DIMENSION(:) :: AWM_AEROSOL,A_LP_MPUA,MASSFLUX_ACTUAL,MASSFLUX,SURFACE_DENSITY,LP_CPUA,LP_MPUA   
!   LOGICAL :: SHRINKING
...
TYPE WALL_TYPE
   ...
   REAL(EB), POINTER, DIMENSION(:) :: VEG_ASHMASS_L,VEG_CHARMASS_L,VEG_FUELMASS_L,VEG_MOISTMASS_L,VEG_TMP_L
   LOGICAL :: LSET_FIRE=.FALSE.
   REAL(EB) :: VEG_HEIGHT,VEG_LSET_SURFACE_HEATFLUX=0._EB
   REAL(EB) :: AREA_ADJUST=1._EB,AW=0._EB,DUWDT=0._EB,E_WALL,EW=0._EB,HEAT_TRANS_COEF=0._EB,KW=1._EB, &
               QCONF=0._EB,QRADIN=0._EB,QRADOUT=0._EB,RAW=0._EB,RDN=1._EB,RHO_F=1._EB,TMP_B=0._EB,TMP_F=0._EB, &
               TW=0._EB,U_TAU=0._EB,UVW_GHOST=0._EB,UW=0._EB,UWS=0._EB,UW0=0._EB,XW=0._EB,YW=0._EB,ZW=0._EB
   REAL(EB), POINTER, DIMENSION(:) :: AWM_AEROSOL,A_LP_MPUA,MASSFLUX_ACTUAL,MASSFLUX,SURFACE_DENSITY,LP_CPUA,LP_MPUA,RHODW,ZZ_F
   INTEGER :: BACK_INDEX=0,BOUNDARY_TYPE=NULL_BOUNDARY,SURF_INDEX_ORIG=0,OBST_INDEX=0,NPPCW=1,PRESSURE_BC_INDEX=NEUMANN,&
              PRESSURE_ZONE_WALL=0,VENT_INDEX=0
   INTEGER :: II=0,JJ=0,KK=0,IIG=0,JJG=0,KKG=0,SURF_INDEX=0,IOR=0,NOM=0,NOM_IB(6)=0
END TYPE WALL_TYPE
...
TYPE REACTION_TYPE
   ...
   LOGICAL :: IDEAL,CHECK_ATOM_BALANCE,REVERSIBLE,NEW_MIX
...
TYPE SURFACE_TYPE
   ...
   REAL(EB), DIMENSION(MAX_LAYERS) :: LAYER_THICKNESS, LAYER_DENSITY, TMP_INNER, STRETCH_FACTOR
   ...
   LOGICAL  :: VEGETATION=.FALSE.,VEG_NO_BURN=.FALSE.,VEG_GROUND_ZERO_RAD=.TRUE., &
               ...
               VEG_UNIT_DRAG_COEFF
   INTEGER  :: NVEG_L
   REAL(EB) :: VEG_ASH_FRACTION,VEG_CHAR_FRACTION,VEG_DRAG_INI,VEG_ELEMENT_DENSITY,VEG_HEIGHT,VEG_INITIAL_TEMP,VEG_LOAD, &
               VEG_MOISTURE,VEG_SV,VEG_PACKING,VEG_KAPPA,FIRELINE_MLR_MAX,VEG_GROUND_TEMP,VEG_POSTFIRE_DRAG_FCTR
   REAL(EB) :: VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
               VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION
   INTEGER,  POINTER, DIMENSION(:) :: VEG_KGAS_L
   ...
   CHARACTER(25) :: VEG_DEGRADATION,VEG_LSET_SURFACE_FIRE_HEAD_ROS_MODEL,VEG_LSET_CROWN_FIRE_HEAD_ROS_MODEL,VEG_LSET_MODEL_FOR_PASSIVE_ROS
   LOGICAL ::  VEG_LSET_SR_CROWNROS_MODEL=.FALSE.,VEG_LSET_HEADWIDTH_DEPENDENCE,VEG_LSET_BURNER,VEG_LSET_SPREAD,VEG_LSET_ELLIPSE, &
               VEG_LSET_TAN2,VEG_LSET_SURFACE_HRRPUA
   INTEGER  :: VEG_LSET_UAVG_K
   REAL(EB) :: VEG_LSET_BURNER_TIME_OFF,VEG_LSET_BURNER_TIME_ON,VEG_LSET_IGNITE_TIME,VEG_LSET_ROS_HEAD,VEG_LSET_ROTH_ZEROWINDSLOPE_ROS, &
               VEG_LSET_ROTHFM10_ZEROWINDSLOPE_ROS,VEG_LSET_QCON,VEG_LSET_ROS_FLANK,VEG_LSET_ROS_BACK, &
               VEG_LSET_WIND_EXP,VEG_LSET_PHIDEPTH,VEG_LSET_HEAT_OF_COMBUSTION,VEG_LSET_SURF_LOAD,VEG_LSET_CHAR_FRACTION, &
               VEG_LSET_SURF_EFFM,VEG_LSET_CANOPY_BULK_DENSITY,VEG_LSET_CANOPY_FMC,VEG_LSET_CANOPY_BASE_HEIGHT,VEG_LSET_CANOPY_HEIGHT, &
               VEG_LSET_FUEL_STRATA_GAP, &
               VEG_LSET_CRUZ_PROB_PASSIVE,VEG_LSET_CRUZ_PROB_ACTIVE,VEG_LSET_CRUZ_PROB_CROWN,VEG_LSET_WAF_UNSHELTERED, &
               VEG_LSET_WAF_SHELTERED,VEG_LSET_CROWNFIRE_ANGLE,VEG_LSET_HRRPUA_MINIMUM_FRAC,VEG_LSET_UAVG_TIME,VEG_LSET_DRAG_INI
...
! For names of files containing HRRPUA data to implement burner method
TYPE BRNR_TYPE
    CHARACTER(256) :: BRNRFILE
END TYPE BRNR_TYPE

TYPE(BRNR_TYPE), POINTER, DIMENSION(:) :: BRNRINFO
...
TYPE OMESH_TYPE
   ...
   TYPE(WALL_ILW_TYPE), POINTER, DIMENSION(:) :: WALL_ILW
   TYPE(PARTICLE_TYPE), POINTER, DIMENSION(:) :: PARTICLE
   ...
   INTEGER , POINTER, DIMENSION(:,:) :: INTG_SEND_PKGW   
...
TYPE INITIALIZATION_TYPE
   ...
   INTEGER  :: PART_INDEX=0,NUMBER_INITIAL_PARTICLES,LU_PARTICLE,PROF_INDEX=0,DEVC_INDEX=0,CTRL_INDEX=0
   LOGICAL :: ADJUST_DENSITY=.FALSE.,ADJUST_TEMPERATURE=.FALSE.,POINTWISE_PARTICLE_INIT=.FALSE., &

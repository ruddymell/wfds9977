...
CHARACTER(255), PARAMETER :: meshid='$Id: mesh.f90 9901 2012-01-25 17:45:17Z randy.mcdermott $'
CHARACTER(255), PARAMETER :: meshrev='$Revision: 9901 $'
CHARACTER(255), PARAMETER :: meshdate='$Date: 2012-01-25 09:45:17 -0800 (Wed, 25 Jan 2012) $'
...
TYPE MESH_TYPE
   ...
   REAL(EB), POINTER, DIMENSION(:,:,:) :: &
            ...
            CSD2,MIX_TIME,STRAIN_RATE,KFST4,RHO_H_S_OVER_PBAR
   ...
   REAL(EB) :: BC_CLOCK
   REAL(EB), POINTER, DIMENSION(:,:) :: EDGE_INTERPOLATION_FACTOR,FW,AWM_AEROSOL
   REAL(EB), POINTER, DIMENSION(:)   :: DUWDT, &
            D_CORR,DS_CORR,UVW_SAVE,U_GHOST,V_GHOST,W_GHOST
...
!  REAL(EB), POINTER, DIMENSION(:,:,:,:) :: DMPVDT_FM_VEG
   ...
   REAL(EB), POINTER, DIMENSION(:,:) :: HEAD_WIDTH,ROS_HEAD,ROS_HEAD_FM10,ROS_HEAD_CROWN,ROS_HEAD_SA,ROS_HEAD_SURF,ROS_FLANK, &
                                        ROS_BACKU,WIND_EXP,FLANKFIRE_LIFETIME
   REAL(EB), POINTER, DIMENSION(:,:) :: PHI_LS,PHI0_LS,PHI1_LS,PHI_TEMP_LS
   REAL(EB), POINTER, DIMENSION(:,:) :: FLUX_LS,FLUX0_LS,FLUX1_LS,SR_X_LS,SR_Y_LS,SR_X_FM10_LS,SR_Y_FM10_LS,SR_X_RSA_LS,SR_Y_RSA_LS, &
                                        SR_X_SURF_LS,SR_Y_SURF_LS
   REAL(EB), POINTER, DIMENSION(:,:) :: U_LS,V_LS,U_LS_AVG,V_LS_AVG,U_LS_SUM,V_LS_SUM
   REAL(EB), POINTER, DIMENSION(:,:) :: BURN_TIME_LS,MASSPUA_CANOPY_CONSUMED
   REAL(EB), POINTER, DIMENSION(:,:) :: ASPECT,DZTDX,DZTDY,MAG_ZT
   REAL(EB), POINTER, DIMENSION(:,:) :: PHI_S,PHI_S_X,PHI_S_Y,PHI_W_X,PHI_W_Y,PHI_W,UMF
   REAL(EB), POINTER, DIMENSION(:,:) :: CFB_SR_LS,PHI_S_FM10,PHI_S_X_FM10,PHI_S_Y_FM10,PHI_W_X_FM10,PHI_W_Y_FM10, &
                                        PHI_W_FM10,UMF_FM10,RAC_THRESHOLD_LS,ROS_FINAL_MAG,ROS_SURF_INI_LS
   REAL(FB), POINTER, DIMENSION(:,:) :: CFB_OUT,CRUZ_CROWN_PROB_OUT,FLI_OUT,HRRPUA_IN,HRRPUA_OUT,MAG_SR_OUT,PHI_OUT,RINI_OUT,ROS_FINAL_MAG_OUT, &
                                        ROS_X_OUT,ROS_Y_OUT,ROS_SURF_X_OUT,ROS_SURF_Y_OUT,ROS10_X_OUT,ROS10_Y_OUT,RSA_X_OUT,RSA_Y_OUT,TOA
   INTEGER,  POINTER, DIMENSION(:,:) :: FLAG_MODEL_FOR_PASSIVE_ROS
   REAL(EB), POINTER, DIMENSION(:,:) :: CRUZ_CROWN_PROB,THETA_ELPS,THETA_ELPS_FM10
   REAL(EB), POINTER, DIMENSION(:,:) :: WORK1_LS,WORK2_LS,WORK3_LS,WORK4_LS
   LOGICAL :: LSET_MESH
   INTEGER :: FIRST_AVG_FLAG_LS,NX_LS,NY_LS,NSUM_T_UAVG_LS
   CHARACTER(80), POINTER, DIMENSION(:) :: FN_SLCF_LS
   INTEGER, POINTER, DIMENSION(:)       :: LU_SLCF_LS
   REAL(EB) :: DT_COEF,DT_LS,DYN_SR_MAX,SUM_T_SLCF_LS,SUM_T_UAVG_LS,SUMTIME_LS,TIME_LS
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: &
          U,V,W,US,VS,WS,DDDT,D,DS,H,HS,KRES,FVX,FVY,FVZ,RHO,RHOS, &
          MU,TMP,Q,FRHO,KAPPA,QR,QR_W,UII,RSUM,D_LAGRANGIAN,D_REACTION, &
          CSD2,MTR,MSR,WEM,MIX_TIME,STRAIN_RATE,KFST4,RHO_H_S_OVER_PBAR
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZ,ZZS,DEL_RHO_D_DEL_Z,DFX,DFY,DFZ
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: AVG_DROP_DEN,AVG_DROP_TMP,AVG_DROP_RAD,AVG_DROP_AREA
REAL(EB), POINTER, DIMENSION(:,:) :: UVW_GHOST
...
INTEGER, POINTER :: FIRST_AVG_FLAG_LS,NX_LS,NY_LS,NSUM_T_UAVG_LS
CHARACTER(80), POINTER, DIMENSION(:) :: FN_SLCF_LS
INTEGER, POINTER, DIMENSION(:)       :: LU_SLCF_LS
REAL(EB), POINTER :: DT_COEF,DT_LS,DYN_SR_MAX,SUM_T_SLCF_LS,SUM_T_UAVG_LS,SUMTIME_LS,TIME_LS
...
H=>M%H 
HS=>M%HS
RHO_H_S_OVER_PBAR=>M%RHO_H_S_OVER_PBAR
KRES=>M%KRES
FVX=>M%FVX
FVY=>M%FVY
FVZ=>M%FVZ  
...
ROS_HEAD      => M%ROS_HEAD
ROS_HEAD_FM10 => M%ROS_HEAD_FM10
ROS_HEAD_CROWN=> M%ROS_HEAD_CROWN
ROS_HEAD_SA   => M%ROS_HEAD_SA  
ROS_HEAD_SURF => M%ROS_HEAD_SURF
...
SR_X_LS       => M%SR_X_LS
SR_Y_LS       => M%SR_Y_LS
SR_X_FM10_LS  => M%SR_X_FM10_LS
SR_Y_FM10_LS  => M%SR_Y_FM10_LS
SR_X_RSA_LS   => M%SR_X_RSA_LS
SR_Y_RSA_LS   => M%SR_Y_RSA_LS
SR_X_SURF_LS  => M%SR_X_SURF_LS
SR_Y_SURF_LS  => M%SR_Y_SURF_LS
U_LS          => M%U_LS
U_LS_AVG      => M%U_LS_AVG
U_LS_SUM      => M%U_LS_SUM
V_LS          => M%V_LS
...
MASSPUA_CANOPY_CONSUMED => M%MASSPUA_CANOPY_CONSUMED
DZTDX         => M%DZTDX
DZTDY         => M%DZTDY
MAG_ZT        => M%MAG_ZT
ASPECT        => M%ASPECT

! For Rothermel surface fire
PHI_S         => M%PHI_S
PHI_S_X       => M%PHI_S_X
PHI_S_Y       => M%PHI_S_Y
PHI_W         => M%PHI_W
...
! For Rothermel using Fuel Model 10 (needed for Scott & Reinhardt crown fire model)
PHI_S_FM10    => M%PHI_S_FM10
PHI_S_X_FM10  => M%PHI_S_X_FM10
PHI_S_Y_FM10  => M%PHI_S_Y_FM10
PHI_W_FM10    => M%PHI_W_FM10
UMF_FM10      => M%UMF_FM10
THETA_ELPS_FM10 => M%THETA_ELPS_FM10

!For Scott and Reinhardt crown fire model
CFB_SR_LS        => M%CFB_SR_LS
RAC_THRESHOLD_LS => M%RAC_THRESHOLD_LS
ROS_SURF_INI_LS  => M%ROS_SURF_INI_LS
ROS_FINAL_MAG    => M%ROS_FINAL_MAG
FLAG_MODEL_FOR_PASSIVE_ROS => M%FLAG_MODEL_FOR_PASSIVE_ROS

!For CFIS crown fire model
CRUZ_CROWN_PROB => M%CRUZ_CROWN_PROB

!For level set output
...
PHI_OUT       => M%PHI_OUT
RINI_OUT      => M%RINI_OUT
ROS_FINAL_MAG_OUT => M%ROS_FINAL_MAG_OUT
ROS_X_OUT     => M%ROS_X_OUT
ROS_Y_OUT     => M%ROS_Y_OUT
ROS_SURF_X_OUT=> M%ROS_SURF_X_OUT
ROS_SURF_Y_OUT=> M%ROS_SURF_Y_OUT
ROS10_X_OUT   => M%ROS10_X_OUT
ROS10_Y_OUT   => M%ROS10_Y_OUT
RSA_X_OUT     => M%RSA_X_OUT
RSA_Y_OUT     => M%RSA_Y_OUT
TOA           => M%TOA
CFB_OUT       => M%CFB_OUT
...
DT_COEF       => M%DT_COEF
DT_LS         => M%DT_LS
DYN_SR_MAX    => M%DYN_SR_MAX
FIRST_AVG_FLAG_LS => M%FIRST_AVG_FLAG_LS
SUM_T_SLCF_LS => M%SUM_T_SLCF_LS
SUM_T_UAVG_LS => M%SUM_T_UAVG_LS
NSUM_T_UAVG_LS=> M%NSUM_T_UAVG_LS

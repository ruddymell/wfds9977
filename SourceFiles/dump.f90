...
DO IW=1,N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_VIRTUAL_WALL_CELLS + N_GHOST_WALL_CELLS
   WRITE(LU_CORE(NM)) WALL(IW)%MASSFLUX
   WRITE(LU_CORE(NM)) WALL(IW)%RHODW
   IF (N_TRACKED_SPECIES>0) WRITE(LU_CORE(NM)) WALL(IW)%ZZ_F
ENDDO
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%QRADIN
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%QRADOUT
...
DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (SURFACE(SURF_INDEX)%THERMALLY_THICK) THEN
      ...
      ELSE
         IF (SURFACE(SURF_INDEX)%N_CELLS>0) WRITE(LU_CORE(NM)) WALL(IW)%TMP_S,WALL(IW)%RHO_S
      ENDIF
   ENDIF
   IF (RADIATION .AND. WALL(IW)%BOUNDARY_TYPE/=OPEN_BOUNDARY) WRITE(LU_CORE(NM)) WALL(IW)%ILW
ENDDO
...
DO IW=1,N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_VIRTUAL_WALL_CELLS + N_GHOST_WALL_CELLS
   READ(LU_RESTART(NM)) WALL(IW)%MASSFLUX
   READ(LU_RESTART(NM)) WALL(IW)%RHODW
   IF (N_TRACKED_SPECIES>0) READ(LU_RESTART(NM))  WALL(IW)%ZZ_F
ENDDO
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%QRADIN
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%QRADOUT
   ...
   IF (SURFACE(SURF_INDEX)%THERMALLY_THICK) THEN
      IF (SURFACE(SURF_INDEX)%SHRINK) THEN
         ...
         IF (SUM(WALL(IW)%N_LAYER_CELLS)>0) THEN
            ...
            ALLOCATE(WALL(IW)%TMP_S(0:SURFACE(SURF_INDEX)%N_CELLS+1))
            NULLIFY(WALL(IW)%RHO_S)
            ALLOCATE(WALL(IW)%RHO_S(0:SURFACE(SURF_INDEX)%N_CELLS+1,SURFACE(SURF_INDEX)%N_MATL))
      ...
      ENDIF
   ENDIF
   IF (RADIATION .AND. WALL(IW)%BOUNDARY_TYPE/=OPEN_BOUNDARY) READ(LU_RESTART(NM)) WALL(IW)%ILW
ENDDO
...
DO NM=1,NMESHES
   ...
   IF (ABS(Q_RADI(NM))>1._EB) WRITE(LU_OUTPUT,120) Q_RADI(NM)/1000._EB
ENDDO
...
PARTICLE_CLASS_LOOP: DO N=1,N_PART
   ...
   LOAD_LOOP: DO I=1,NLP
      ...
      YP(NPP) = LP%Y
      ZP(NPP) = LP%Z
      CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XI,YJ,ZK,II,JJ,KK) !for computing V_CELL for Veg output
      V_CELL = DX(II)*DY(JJ)*DZ(KK)
      DO NN=1,PC%N_QUANTITIES
         SELECT CASE(PC%QUANTITIES_INDEX(NN))
            CASE( 6)  ! U-VELOCITY
               QP(NPP,NN) = LP%U
!              IF (PC%TREE .AND. .NOT. LP%IGNITOR) QP(NPP,NN) = LP%VEG_FUEL_MASS*V_CELL*LP%VEG_VOLFRACTION
            CASE( 7)  ! V-VELOCITY
               QP(NPP,NN) = LP%V
               IF (PC%TREE .AND. .NOT. LP%IGNITOR) QP(NPP,NN) = LP%VEG_MOIST_MASS*V_CELL*LP%VEG_VOLFRACTION
            CASE( 8)  ! W-VELOCITY
               QP(NPP,NN) = LP%W
               IF (PC%TREE .AND. .NOT. LP%IGNITOR) QP(NPP,NN) = LP%VEG_DIVQC*V_CELL*LP%VEG_VOLFRACTION
            CASE(434)  ! PARTICLE DIAMETER
               QP(NPP,NN) = 2.E6*LP%R
               IF (PC%TREE .AND. .NOT. LP%IGNITOR) QP(NPP,NN) = LP%VEG_DIVQR*V_CELL*LP%VEG_VOLFRACTION
            ...
            CASE(440)  ! PARTICLE WEIGHTING FACTOR
               QP(NPP,NN) = LP%PWT
            CASE(441)  ! VEG_DRY_MASS
               QP(NPP,NN) = LP%VEG_FUEL_MASS*V_CELL*LP%VEG_VOLFRACTION !kg
            CASE(442)  ! VEG_MOIST_MASS
               QP(NPP,NN) = LP%VEG_MOIST_MASS*V_CELL*LP%VEG_VOLFRACTION !kg
            CASE(443)  ! VEG_DIVQC
               QP(NPP,NN) = LP%VEG_DIVQC*V_CELL*0.001_EB !kW
            CASE(444)  ! VEG_DIVQR
               QP(NPP,NN) = LP%VEG_DIVQR*V_CELL*0.001_EB !kW
            CASE(445)  ! VEG_CHAR_MASS
               QP(NPP,NN) = LP%VEG_CHAR_MASS*V_CELL*LP%VEG_VOLFRACTION !kg
            CASE(446)  ! VEG_ASH_MASS
               QP(NPP,NN) = LP%VEG_ASH_MASS*V_CELL*LP%VEG_VOLFRACTION !kg
            CASE(447)  ! VEG_Q_CHAROX
               QP(NPP,NN) = LP%VEG_Q_CHAROX*V_CELL*0.001_EB !kW
         END SELECT
      ENDDO
   ENDDO LOAD_LOOP
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,&
            ...
            DHOR,X_EQUIL,MW_RATIO,Y_EQUIL,TMP_BOIL,EXPON,Y_SPECIES,MEC,Y_SPECIES2,Y_H2O,R_Y_H2O,R_DN,SGN,Y_ALL(N_SPECIES),H_S
...
SELECT CASE(INDX)
   CASE( 1) ! RADIATIVE HEAT FLUX
      SOLID_PHASE_OUTPUT = (WC%QRADIN-WC%QRADOUT)*0.001_EB
   CASE( 2) ! CONVECTIVE HEAT FLUX
      SOLID_PHASE_OUTPUT = WC%QCONF*0.001_EB
   ...
   CASE( 6) ! INSIDE WALL TEMPERATURE
      IF (SURFACE(WC%SURF_INDEX)%SHRINK) THEN
         II2 = SUM(WC%N_LAYER_CELLS)
         IF (DV%DEPTH>WC%X_S(II2)) THEN
            SOLID_PHASE_OUTPUT = TMPA-TMPM
         ELSE
            DO II2=II2,1,-1
               IF (DV%DEPTH<=WC%X_S(II2)) II1 = II2
            ENDDO
            SOLID_PHASE_OUTPUT = WC%TMP_S(II1)-TMPM
         ENDIF
      ELSE
         SOLID_PHASE_OUTPUT = WC%TMP_S(DV%I_DEPTH) - TMPM
      ENDIF

   CASE( 7) ! BURNING RATE
      IF (N_REACTIONS>0) THEN
         SOLID_PHASE_OUTPUT = WC%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   CASE( 8) ! HRRPUM
      SOLID_PHASE_OUTPUT = WC%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION/ &
                           SURFACE(WC%SURF_INDEX)%SURFACE_DENSITY
   CASE( 9) ! HRRPUA
      SOLID_PHASE_OUTPUT = WC%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION
   CASE(10) ! NET HEAT FLUX
      SOLID_PHASE_OUTPUT = (WC%QRADIN-WC%QRADOUT+WC%QCONF)*0.001_EB
   ...
   CASE(13) ! GAUGE HEAT FLUX
      CONCORR = WC%HEAT_TRANS_COEF*(WC%TMP_F-PY%GAUGE_TEMPERATURE)
      SOLID_PHASE_OUTPUT = (WC%QRADIN/(WC%E_WALL+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4 + &
                            WC%QCONF + CONCORR)*0.001_EB
   CASE(19) ! INCIDENT HEAT FLUX
      SOLID_PHASE_OUTPUT = ( WC%QRADIN/(WC%E_WALL+1.0E-10_EB) + WC%QCONF )*0.001_EB
   CASE(20) ! MASS FLUX
     IF (Z_INDEX >=0) THEN
        SOLID_PHASE_OUTPUT = WC%MASSFLUX(Z_INDEX)
     ELSEIF(Y_INDEX >0) THEN
        ZZ_GET(1:N_TRACKED_SPECIES) = WC%ZZ_F(1:N_TRACKED_SPECIES)
        CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
        SOLID_PHASE_OUTPUT = SUM(WC%MASSFLUX)*Y_SPECIES
     ENDIF
   CASE(21) ! HEAT TRANSFER COEFFICENT
      SOLID_PHASE_OUTPUT = WC%HEAT_TRANS_COEF
   CASE(22) ! RADIOMETER
      SOLID_PHASE_OUTPUT = (WC%QRADIN/(WC%E_WALL+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4)*0.001_EB
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE
      SOLID_PHASE_OUTPUT_OLD = WC%TMP_F
      SOLID_PHASE_OUTPUT     = WC%TMP_F
      ADLOOP: DO
         CONCORR = WC%HEAT_TRANS_COEF*(WC%TMP_F-SOLID_PHASE_OUTPUT_OLD)      
         FLUX = WC%QCONF + CONCORR + WC%QRADIN - WC%E_WALL * SIGMA * SOLID_PHASE_OUTPUT_OLD ** 4
         DFLUXDT = -4._EB*WC%E_WALL*SIGMA*SOLID_PHASE_OUTPUT_OLD**3 - WC%HEAT_TRANS_COEF
   ...
   CASE(24) ! WALL THICKNESS
      IF (SURFACE(WC%SURF_INDEX)%SHRINK) THEN
         SOLID_PHASE_OUTPUT = SUM(WC%LAYER_THICKNESS)
      ELSE
   ...
   CASE(26) ! SOLID DENSITY
      ...
      DO NN=1,SF%N_MATL
         IF (DV%MATL_ID==SF%MATL_NAME(NN)) THEN
            SOLID_PHASE_OUTPUT = WC%RHO_S(II1,NN)
            RETURN
         ENDIF
      ENDDO
      SOLID_PHASE_OUTPUT = 0._EB
   ...
   CASE(30) ! MPUA
      PC => PARTICLE_CLASS(PART_INDEX)
      SOLID_PHASE_OUTPUT = WC%LP_MPUA(PC%EVAP_INDEX)
   CASE(31) ! CPUA
      PC => PARTICLE_CLASS(PART_INDEX)
      SOLID_PHASE_OUTPUT = WC%LP_CPUA(PC%EVAP_INDEX)*0.001_EB
   CASE(32) ! AMPUA
      PC => PARTICLE_CLASS(PART_INDEX)
      SOLID_PHASE_OUTPUT = WC%A_LP_MPUA(PC%EVAP_INDEX)
   CASE(33) ! SOLID SPECIFIC HEAT
      ...
      MATERIAL_LOOP_CP: DO NN=1,SF%N_MATL
         IF (WC%RHO_S(II1,NN)<=ZERO_P) CYCLE MATERIAL_LOOP_CP
         ML  => MATERIAL(SF%MATL_INDEX(NN))
         VOLSUM = VOLSUM + WC%RHO_S(II1,NN)/ML%RHO_S         
         IF (ML%C_S>0._EB) THEN
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%RHO_S(II1,NN)*ML%C_S/ML%RHO_S
         ELSE
            NR     = -NINT(ML%C_S)
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%RHO_S(II1,NN)*EVALUATE_RAMP(WC%TMP_S(II1),0._EB,NR)* &
   ...
   CASE(34) ! SOLID CONDUCTIVITY 
      ...
      MATERIAL_LOOP_K: DO NN=1,SF%N_MATL
         IF (WC%RHO_S(II1,NN)<=ZERO_P) CYCLE MATERIAL_LOOP_K
         ML  => MATERIAL(SF%MATL_INDEX(NN))
         VOLSUM = VOLSUM + WC%RHO_S(II1,NN)/ML%RHO_S         
         IF (ML%K_S>0._EB) THEN  
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%RHO_S(II1,NN)*ML%K_S/ML%RHO_S
         ELSE
            NR     = -NINT(ML%K_S)
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%RHO_S(II1,NN)*EVALUATE_RAMP(WC%TMP_S(II1),0._EB,NR)/ML%RHO_S
         ENDIF
...
REAL(EB) :: VC,TMP_N,RHO_N,U_N,ZZ_GET(0:N_TRACKED_SPECIES),E_DIFF,H_S,HDIFF,H_S_J_ALPHA,CP
INTEGER, INTENT(IN) :: NM
...
DO K=1,KBAR
   DO J=1,JBAR
      ILOOP2: DO I=1,IBAR
         ...
         Q_RADI(NM) = Q_RADI(NM) - QR(I,J,K)*VC
      ENDDO ILOOP2
   ENDDO
ENDDO
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   ENDDO
   E_DIFF = H_S_J_ALPHA + WC%QCONF
   IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY)  Q_OPEN(NM) = Q_OPEN(NM) + (U_N*RHO_N*H_S+E_DIFF)*WC%AW
   IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) Q_WALL(NM) = Q_WALL(NM) + (U_N*RHO_N*H_S+E_DIFF)*WC%AW
ENDDO WALL_LOOP

! Determine mass loss rate (MLR) of fuel
...
IF (N_REACTIONS > 0) THEN
   WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
      ...
      MLR(NM) = MLR(NM) + WC%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)*WC%AW*WC%AREA_ADJUST
   ENDDO WALL_LOOP2
ENDIF

HRR_SUM(NM)  = HRR_SUM(NM)  + DT*HRR(NM)
Q_RADI_SUM(NM) = Q_RADI_SUM(NM) + DT*Q_RADI(NM)
Q_WALL_SUM(NM) = Q_WALL_SUM(NM) + DT*Q_WALL(NM)
Q_OPEN_SUM(NM) = Q_OPEN_SUM(NM) + DT*Q_OPEN(NM)
...
REAL(FB) :: STIME
INTEGER :: NM,I
REAL(EB) :: HRR_TOTAL,Q_RADI_TOTAL,Q_WALL_TOTAL,Q_OPEN_TOTAL,MLR_TOTAL
...
DO NM=1,NMESHES
   IF (HRR_TIME_INTERVAL(NM)>0._EB) THEN
      ...
      Q_RADI_TOTAL = Q_RADI_TOTAL + Q_RADI_SUM(NM)/HRR_TIME_INTERVAL(NM)
      Q_WALL_TOTAL = Q_WALL_TOTAL + Q_WALL_SUM(NM)/HRR_TIME_INTERVAL(NM)
      Q_OPEN_TOTAL = Q_OPEN_TOTAL + Q_OPEN_SUM(NM)/HRR_TIME_INTERVAL(NM)
      MLR_TOTAL    = MLR_TOTAL    + MLR_SUM(NM)/HRR_TIME_INTERVAL(NM)
   ENDIF
ENDDO
 
WRITE(TCFORM,'(A,I4.4,5A)') "(",6+N_ZONE,"(",FMT_R,",','),",FMT_R,")"
WRITE(LU_HRR,TCFORM) STIME,0.001_EB*HRR_TOTAL,.001_EB*Q_RADI_TOTAL,.001_EB*Q_OPEN_TOTAL,.001_EB*Q_WALL_TOTAL, & 
                     0.001_EB*(Q_RADI_TOTAL+Q_OPEN_TOTAL+Q_WALL_TOTAL),MLR_TOTAL,(MESHES(1)%PBAR(1,I),I=1,N_ZONE)
...
IF (VEG_PRESENT) THEN
 DO I=1,N_TREES_OUT
  ...
  IF (NPRTS_TREE == 0) THEN
    NPRTS_TREE = 1
    TREE_OUTPUT_DATA_TOTAL(I,1) = 0.0_EB
    TREE_OUTPUT_DATA_TOTAL(I,2) = 0.0_EB
    TREE_OUTPUT_DATA_TOTAL(I,7) = 0.0_EB
    TREE_OUTPUT_DATA_TOTAL(I,8) = 0.0_EB
  ENDIF
